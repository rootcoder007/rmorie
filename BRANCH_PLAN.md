...
| 31 | MASS models: glm.nb (NB GLM, 6 sites) + kde2d (2-D KDE, 3 sites) + rlm (Huber M-est, 1 site) + polr (ordered logit, 1 site) + negative.binomial family | MASS | DONE (glm.nb/kde2d/rlm bit-for-bit vs MASS incl coef/theta/summary-SE/logLik/AIC/predict; polr logLik matches to 1e-6 optim-tol; negbin family drives glm.nb + survey NB; 12 sites rewired native, last MASS functional call gone; MASS now Suggests-only for tests/cross; +summary.negbin/logLik.negbin/summary.morie_rlm/logLik.morie_polr S3 methods) |
