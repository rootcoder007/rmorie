...
#' @references Schabenberger & Gotway (2005), Sec 4.3.2, eq (4.11), p. 144.
#' @examples
#' spexp(h = c(0, 0.5, 1, 2), nugget = 0.1, sill = 1, range = 1)
#' @export
spexp <- function(h, nugget = 0, sill = 1, range = 1) {
  g <- .sp_semivariogram(h, nugget, sill, range, "exponential")
  list(gamma = g, nugget = as.numeric(nugget), sill = as.numeric(sill),
       range = as.numeric(range), model = "exponential")
}
