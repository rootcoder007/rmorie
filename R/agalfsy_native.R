...
morie_agalfsy_rl_pose_search <- function(receptor, ligand, site = NULL,
  ...
  kind <- if (!is.null(policy)) {
    "supplied policy"
  } else {
    "greedy oracle (needs the answer; benchmark only)"
  }
  ...
  for (step in seq_len(as.integer(max_steps))) {
    if (!is.null(policy)) {
      ...
      if (!(a >= 0L && a < 12L)) {
        stop(sprintf(paste0(
          "agalfsy: policy returned action %d, the action ",
          "space is 0..11"
        ), a))
      }
    ...
    cv <- if (!is.null(critic)) {
      as.numeric(critic(L, R, step))
    ...
    if (step >= as.integer(min_steps) &&
      ...
      w <- crit_hist[seq.int(
        length(crit_hist) - as.integer(window) + 1L,
        length(crit_hist)
      )]
  ...
  dcc <- if (!is.null(S)) {
    sqrt(sum((.agalfsy_centroid(L) - .agalfsy_centroid(S))^2))
  } else {
    NA_real_
  }
