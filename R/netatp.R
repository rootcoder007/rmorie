...
Netattack <- function(A, strategy = "attack", k = 1, seed = 1) {
  ...
  tot <- 0; pairs <- 0L
  for (s in seq_len(m)) {
    dist <- rep(-1L, m); dist[s] <- 0L; q <- s; h <- 1L
    while (h <= length(q)) {
      v <- q[h]; h <- h + 1L
      w <- which(B[v, ] & dist < 0L)
    ...
    d <- dist[-s]
    tot <- tot + sum(d[d > 0]); pairs <- pairs + sum(d > 0)
  }
