...
#'
#' Formula: G = L L';  Zstar = Z L;  then Y = X beta + Zstar ustar + e with ustar ~ N(0,
#' sigma2_g I) has the same marginal variance as u ~ N(0, sigma2_g G)
#'
...
#'
#' @return List with ``Zstar``, ``L``, ``V_original``, ``V_reparameterized``,
#' ``max_gap``, ``n``, ``q``.
...
#' doi:10.1007/978-3-030-89010-0.  Chapter 2, p. 46: the GBLUP model can be expressed
#' equivalently as Y = X beta + Z* u + e with Z* = Z L', G = L'L the Cholesky
#' decomposition of G, and u ~ N(0, sigma2_g I_q).  The book's L is the upper factor
#' returned by R's chol(); this implementation uses the lower factor L with G = L L' and
#' Z* = Z L in BOTH language arms, which gives the identical marginal variance sigma2_g Z
#' G Z' -- ``max_gap`` reports the largest entry-wise difference between the two marginal
#' variances, and is zero up to rounding.  Read from the chapter PDF, not recalled.
#' @export
#' @examples
#' set.seed(1)
