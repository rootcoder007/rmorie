...
#'
#' Formula: r = sum (yhat - mean yhat)(y - mean y) / sqrt(sum (yhat - mean yhat)^2 * sum
#' (y - mean y)^2)
#'
...
#' doi:10.1007/978-3-030-89010-0.  Chapter 4, Sect. 4.5.1, Eq. (4.1) p. 129 (MSE), Eq.
#' (4.2) p. 129 (Pearson accuracy) and Eq. (4.3) p. 131 (MAE).  Read from the chapter
#' PDF, not recalled.  Eq. (4.2) is Pearson's correlation between the T test-set
#' predictions and the T observed values; the book calls it the prediction accuracy in
#' plant breeding.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
