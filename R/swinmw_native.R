...
Swinmw <- function(x, window_size, relative_bias = NULL,
  ...
  dm <- dim(A); H <- dm[1L]; W <- dm[2L]; d <- dm[3L]
  ...
  Wq <- if (is.null(WQ)) diag(d) else { m <- as.matrix(WQ); storage.mode(m) <- "double"; m }
  Wk <- if (is.null(WK)) diag(d) else { m <- as.matrix(WK); storage.mode(m) <- "double"; m }
  Wv <- if (is.null(WV)) diag(d) else { m <- as.matrix(WV); storage.mode(m) <- "double"; m }
  ...
  dk <- ncol(Wq); dv <- ncol(Wv)
  if (is.null(relative_bias)) {
    tab <- matrix(0, 2L * M - 1L, 2L * M - 1L)
  } else {
    tab <- as.matrix(relative_bias); storage.mode(tab) <- "double"
  ...
  for (h0 in seq(0L, H - M, by = M)) {
    for (w0 in seq(0L, W - M, by = M)) {
      ...
      Wt <- t(apply(S, 1L, function(row) {
        mx <- max(row); e <- exp(row - mx); e / sum(e)
      }))
