...
#' chapter PDF, not recalled.  BOOK DEFECT: the display in Sect. 10.7.2 prints this loss
#' without its leading minus sign, even though the same paragraph calls it the negative
#' log-likelihood of a product of Bernoulli distributions and the Poisson loss two
#' displays later does carry its sign.  What is implemented here is the quantity the
#' surrounding text requires -- a loss that is minimised -- not the sign-dropped display.
#'  The book has not been silently corrected elsewhere.
#' @export
#' @examples
#' Bcelo(Y = 5L, P = 0.5)
Bcelo <- function(Y, P) {
