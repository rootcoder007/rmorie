...
.phylml_cheatsheet <- function() {
  return(paste0(
    ...
    "ren of sum_x P_sx(v) L_x(child); tips are 0/1 indicators; L ",
    "= sum_s pi_s L_s(root) (eq. 5). Turns a 2^(2n-2)-term sum in",
    "to a linear traversal. F81 model P_ij(t) = e^-ut delta_ij + ",
    "(1-e^-ut) pi_j (eq. 7), reversible, and the PULLEY PRINCIPLE",
    " means the two root branches matter only through their sum."
  ))
}
