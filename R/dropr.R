...
#' @param mask Keep/drop indicator per unit: 1 keeps, 0 drops.  Supplied by the caller so
...
#' doi:10.1007/978-3-030-89010-0.  Chapter 10, Sect. 10.6 p. 404 describes dropout as
#' setting a random fraction of the weights of the input or hidden neurons to zero, so
#' their contribution is removed on the forward pass and they receive no update on the
#' backward pass; it prints no formula and attributes the method to Srivastava, Hinton,
#' Krizhevsky, Sutskever and Salakhutdinov (2014), Dropout: A Simple Way to Prevent
#' Neural Networks from Overfitting, JMLR 15:1929-1958, which is where the 1/(1 - rate)
#' inverted scaling comes from.  The mask is an argument rather than drawn internally so
#' the function is deterministic.  Chapter read from the PDF; the scaling is from the
#' paper the book names.
#' @export
#' @examples
#' set.seed(1)
