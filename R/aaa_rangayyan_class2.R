...
.morie_bx_omp <- function(x, D, sparsity, tol) {
  ...
  norms <- vapply(
    seq_len(nrow(D)), function(j) .morie_bx_nrm(D[j, ]),
    numeric(1)
  )
...
.morie_bx_stft <- function(x, nwin, hop) {
  ...
  list(
    re = do.call(rbind, re_f), im = do.call(rbind, im_f),
...
.morie_bx_scores <- function(tp, tn, fp, fn) {
  tot <- tp + tn + fp + fn
  list(
    sensitivity = if (tp + fn > 0) tp / (tp + fn) else NaN,
...
MlpBp <- function(X, y, hidden = 4, eta = 0.5, alpha = 0.9, maxiter = 500,
  ...
  for (s in seq_len(n)) {
    ...
    raw[s, ] <- yo
    pred[s] <- if (K == 1L) {
      (if (yo[1L] >= 0.5) labels[2L] else labels[1L])
    } else {
      labels[which.max(yo)]
    }
  }
...
Bbb <- function(qrsdur, criteria = NULL) {
  ...
  block <- if (left && right) {
    ...
    "QRS wider than 120 ms, complete block not excluded"
  } else if (wide) {
    "QRS wider than normal, block criteria not met"
  } else {
    "no bundle-branch block by these criteria"
  }

  list(
    blocktype = block, qrsdur = qrsdur, wide = wide, left = left,
    right = right,
    satisfied = list(left = left_parts, right = right_parts),
    method = paste0(
      "incomplete bundle-branch block decision rules, ",
      "Rangayyan Biomedical Signal Analysis 3rd ed. ",
      "Section 10.2.1"
    )
  )
...
PvcBayes <- function(features, labels, priors = NULL, query = NULL) {
  ...
  for (ci in seq_along(classes)) {
    ...
    if (nrow(rows) < p + 1L) {
      stop(
        "each class needs more rows than features to estimate a ",
        "covariance matrix"
      )
    }
  ...
  cf <- .morie_bx_confusion(
    as.integer(y == classes[2L]),
  ...
  list(
    predictions = pred, queryclass = qcls, posterior = P, means = means,
    ...
    method = paste0(
      "Gaussian Bayes classifier on [QRSTA, FF] beat ",
...
.morie_bx_chsel <- function(trials, nselect, rank, maxiter, tol, seed) {
  ...
  if (rank < 3L) {
    stop(
      "rank must be at least 3: with r = 2 the min-max normalisation ",
  ...
  list(
    X = X, C = C, W = W, H = fac$H, error = fac$error, rmsd = rmsd,
...
BciChSel <- function(trials, nselect, rank = 4, maxiter = 200, tol = 1e-8,
  ...
  list(
    selected = r$selected, weights = r$rmsd[sel], rmsd = r$rmsd,
    normalized = r$normalized, weighted = weighted, W = r$W, H = r$H,
    covariance = r$C, error = r$error,
    method = paste0(
      "NMF-based EEG channel selection and weighting for ",
...
BPursuit <- function(x, D, lam = 0.01, maxiter = 2000, tol = 1e-10) {
  ...
  l1 <- .morie_fsum(abs(a))
  list(
    alpha = a, support = which(a != 0) - 1L, reconstruction = approx,
    residual = r, l1norm = l1,
    objective = 0.5 * .morie_fsum(r * r) + lam * l1, iterations = it,
    method = paste0(
      "basis-pursuit denoising by iterative soft ",
...
CadPipe <- function(features, labels, k = 5, standardize = TRUE) {
  ...
  for (f in seq_len(k)) {
    ...
    proto <- lapply(c(0L, 1L), function(cc) {
      ...
      vapply(seq_len(p), function(j) {
        .morie_fsum(F[rows, j] / sc[j]) / length(rows)
      }, numeric(1))
    })
    for (i in test) {
      z <- F[i, ] / sc
  ...
  list(
    ...
    method = paste0(
      "cross-validated prototype-discriminant CAD ",
...
CnnSig <- function(x, kernels, bias = NULL, pool = 2, dense = NULL) {
  ...
  for (ki in seq_len(nrow(K))) {
    ...
    conv <- vapply(seq_len(length(x) - m + 1L), function(i) {
      max(0, .morie_fsum(w * x[i + seq_len(m) - 1L]) + b[ki])
    }, numeric(1))
    maps[[ki]] <- conv
    st <- if (length(conv) >= pool) {
      seq(1L, length(conv) - pool + 1L, by = pool)
    } else {
      integer(0)
    }
    pl <- if (length(st)) {
  ...
  if (!is.null(dense)) {
    ...
    if (ncol(Wd) != length(feat)) {
      stop(
        "dense rows must match the pooled feature length of ",
        length(feat)
      )
    }
  ...
  list(
    maps = maps, pooled = pooled, features = feat, scores = scores,
    predicted = best,
    method = paste0(
      "1-D convolution, rectifier and max-pooling forward ",
...
FecgNmf <- function(x, fs, nwin = 64, hop = NULL, rank = 4, lam = 0,
  ...
  if (as.numeric(lam) > 0) {
    ...
    for (i in seq_len(20L)) {
      R <- .morie_bx_mm(W, H)
      for (a in seq_len(nrow(H))) {
        for (bb in seq_len(ncol(H))) {
  ...
  cnt <- vapply(
    seq_len(nrow(H)), function(a) peakcount(H[a, ], tauf),
    numeric(1)
  )
  ...
  rebuild <- function(row) {
    ...
    for (fi in seq_len(nrow(sp$mag))) {
      for (k in seq_len(ncol(sp$mag))) {
        tot <- R[k, fi]
  ...
  list(
    fetal = rebuild(frow), maternal = rebuild(mrow), fetalrow = frow,
    maternalrow = mrow, peaks = peaks, W = W, H = H, error = fac$error,
    method = paste0(
      "single-channel fetal ECG extraction by NMF of the ",
...
PvcLinDf <- function(rr, ff, train = NULL) {
  ...
  disc <- a * rr + b * ff + cc
  list(
    labels = as.integer(disc <= 0), discriminant = disc,
    coefficients = list(rr = a, ff = b, constant = cc),
    prototypes = proto, source = src,
    method = paste0(
      "linear discriminant on [RR interval, form factor] ",
...
EegBands <- function(x, fs, bands = NULL) {
  ...
  list(
    power = power, fraction = frac,
    dominant = if (total > 0) {
      names(frac)[which.max(unlist(frac))]
    } else {
      NULL
    },
    ...
    method = paste0(
      "fractional power in the EEG rhythm bands, band ",
...
SeizDict <- function(signals, labels, iterations = 7, atoms = NULL,
  ...
  if (!length(trained)) {
    stop(
      "dictionary learning produced no atoms; check that the signals ",
      "are not all zero"
    )
  }
  ...
  if (!is.null(test)) {
    ...
    tcls <- vapply(seq_len(nrow(Tm)), function(i) {
      assign1(feats(Tm[i, ])$v)
    }, numeric(1))
  }

  list(
    dictionary = Dm, coefficients = lapply(Ftr, function(f) f$co),
    error = vapply(Ftr, function(f) f$err, numeric(1)),
    predictions = pred, isseizure = pred == max(classes),
    testclass = tcls, accuracy = acc,
    method = paste0(
      "signal-derived dictionary learning (Algorithm 9.2) ",
...
IcaFix <- function(X, ncomp = NULL, maxiter = 200, tol = 1e-8, seed = 1) {
  ...
  for (cix in seq_len(L)) {
    ...
    for (it in seq_len(maxiter)) {
      ...
      g <- vapply(seq_len(L), function(i) {
        .morie_bx_nsum(Z[i, ] * gv)
      }, numeric(1))
      gp <- .morie_bx_nsum(1 - gv * gv)
      wn <- g / T - (gp / T) * w
      if (nrow(Wm)) {
        for (pi_ in seq_len(nrow(Wm))) {
      ...
      wn <- wn / nr
      if (abs(abs(.morie_bx_dot(wn, w)) - 1) < tol) {
        w <- wn
        break
      }
      w <- wn
  ...
  Ginv <- vapply(seq_len(L), function(j) {
    .morie_bx_solve(G, as.numeric(seq_len(L) == j))
  }, numeric(L))
  A <- matrix(0, K, L)
  for (i in seq_len(K)) {
    for (j in seq_len(L)) {
  ...
  list(
    sources = S, unmixing = W, mixing = A, whitening = Wh, mean = mu,
    iterations = iters,
    method = paste0(
      "FastICA fixed-point ICA with the tanh ",
...
IcaClean <- function(X, ncomp = NULL, kurtosis = 3, drop = NULL,
  ...
  gone <- if (length(art)) {
    .morie_fsum(vapply(art + 1L, function(c) {
      .morie_fsum(S[c, ]^2)
    }, numeric(1)))
  } else {
    0
  ...
  list(
    clean = clean, components = S, kurtosis = kv, artifacts = art,
    mixing = A, removedpower = if (orig > 0) gone / orig else 0,
    method = paste0(
      "ICA artifact removal by zeroing high-kurtosis ",
...
Infomax <- function(X, ncomp = NULL, eta = 0.05, maxiter = 300, tol = 1e-8,
  ...
  list(
    sources = .morie_bx_mm(W, Z), unmixing = .morie_bx_mm(W, Wh),
    whitening = Wh, mean = mu, iterations = it, change = chg,
    method = paste0(
      "Infomax ICA with the natural gradient; unmixing ",
...
VagClass <- function(segments, durations = NULL, segclass = NULL,
  ...
  smeans <- vapply(
    seq_len(ns), function(i) .morie_bx_mean(S[i, ]),
    numeric(1)
  )
  ...
  list(
    varmeans = vms, segmentmeans = smeans, abnormalfraction = fabn,
    normalfraction = fnor, decision = dec, stage = stage, abnormal = abn,
    durations = d,
    method = paste0(
      "VAG cartilage-pathology screening: variance of ",
...
KsvdFit <- function(Y, natoms, sparsity, maxiter = 15, tol = 1e-10,
  ...
  for (k in seq_len(natoms)) {
    ...
    a <- if (k0 < m) {
      ...
      vapply(seq_len(n), function(i) u() - 0.5, numeric(1))
    }
  ...
  list(
    dictionary = D, coefficients = Xc, error = err, iterations = it,
    method = paste0(
...
DictCode <- function(Y, D, sparsity, tol = 1e-12) {
  ...
  for (i in seq_len(nrow(S))) {
    ...
    recs[[i]] <- vapply(seq_len(n), function(t) {
      .morie_fsum(r$coefficients * A[, t])
    }, numeric(1))
    res[[i]] <- r$residual
  }
  err <- sqrt(.morie_fsum(unlist(res, use.names = FALSE)^2))
  list(
    coefficients = coefs, support = sups, reconstruction = recs,
    residual = res, error = err,
    method = paste0(
      "sparse coding of signals in a fixed dictionary by ",
...
Lstm <- function(sequences, labels = NULL, hidden = 8, ridge = 1e-6,
  ...
  if (!is.null(labels)) {
    ...
    pred <- vapply(seq_along(y), function(i) {
      classes[which.max(.morie_bx_mv(read, A[i, ]))]
    }, numeric(1))
    acc <- sum(y == pred) / length(y)
  }

  list(
    hidden = hs, cell = cs, predictions = pred, accuracy = acc,
    readout = read, classes = classes,
    method = paste0(
      "LSTM recurrence with a ridge least-squares readout ",
...
MPursuit <- function(x, dictionary = NULL, natoms = 20, tol = 1e-10,
  ...
  rec <- vapply(seq_len(n), function(i) {
    .morie_fsum(coef * D[idx, i])
  }, numeric(1))
  list(
    coefficients = coef, atoms = D[idx, , drop = FALSE],
    indices = idx - 1L, residual = r, reconstruction = rec,
    decay = decay, energyratio = 1 - e_prev / e0,
    parameters = params[idx],
    method = paste0(
      "matching-pursuit decomposition into time-frequency ",
...
BmiDec <- function(y, C, a = NULL, procnoise = 1e-4, obsnoise = 1e-2,
  ...
  for (t in seq_len(nrow(Y))) {
    ...
    Kg <- vapply(seq_len(K), function(col) {
      .morie_bx_solve(Sm, as.numeric(seq_len(K) == col))
    }, numeric(K))
    Kmat <- matrix(0, L, K)
    for (i in seq_len(L)) {
      for (j in seq_len(K)) {
  ...
  list(
    states = states, innovations = innov, gain = gains,
    predicted = preds,
    method = paste0(
      "Kalman-filter neural decoder for prosthesis ",
...
NmfMu <- function(V, r, maxiter = 200, tol = 1e-10, cost = "ls", seed = 1) {
  ...
  if (identical(cost, "kld") && any(M <= 0)) {
    stop(
      "the divergence cost is undefined where V has a zero element; ",
      "use cost='ls'"
    )
  }
  fac <- .morie_bx_nmfmu(
    M, r, as.integer(maxiter), as.numeric(tol), seed,
    cost
  )
  W <- fac$W
  H <- fac$H
  ...
  list(
    W = W, H = H, submatrices = subs, error = fac$error,
    iterations = fac$iterations, cost = cost,
    method = paste0(
      "nonnegative matrix factorisation by multiplicative ",
...
NmfChSel <- function(trials, nselect, rank = 4, maxiter = 200, tol = 1e-8,
  ...
  r <- .morie_bx_chsel(trials, nselect, rank, maxiter, tol, seed)
  list(
    selected = r$selected, rmsd = r$rmsd, ranking = r$ranking,
    normalized = r$normalized, W = r$W, H = r$H, covariance = r$C,
    error = r$error,
    method = paste0(
      "NMF-based EEG channel ranking by normalised ",
...
OmpFit <- function(x, D, sparsity = NULL, tol = 1e-10) {
  ...
  rec <- vapply(seq_len(n), function(i) {
    .morie_fsum(r$coefficients * A[, i])
  }, numeric(1))
  err <- .morie_bx_nrm(r$residual)
  list(
    coefficients = r$coefficients, support = r$support - 1L,
    reconstruction = rec, residual = r$residual, error = err,
    energyratio = 1 - (err * err) / e0,
    method = paste0(
      "orthogonal matching pursuit; Pati, Rezaiifar and ",
...
PcaSig <- function(X, ncomp = NULL) {
  ...
  list(
    components = P, eigenvalues = ev$values, eigenvectors = ev$vectors,
    mean = mu, covariance = S,
    varexplained = if (tot > 0) {
      pmax(0, ev$values[seq_len(L)]) / tot
    } else {
      numeric(L)
    },
    mse = if (L < K) .morie_fsum(pmax(0, ev$values[(L + 1L):K])) else 0,
    method = paste0(
      "principal component analysis of signal mixtures by ",
...
MixCmp <- function(X, ncomp = NULL, maxiter = 200, seed = 1) {
  ...
  list(
    error = err, best = names(err)[which.min(unlist(err))],
    components = list(
      pca = p$components, ica = ic$sources,
      nmf = list(W = nm$W, H = nm$H)
    ),
    rank = L,
    method = paste0(
      "comparison of PCA, ICA and NMF by relative ",
...
Rbfn <- function(X, y, ncenters = NULL, spread = 1, centers = NULL,
  ...
  if (!is.null(centers)) {
    ...
    for (step in seq_len(J)) {
      ...
      for (i in seq_len(n)) {
        if (i %in% chosen) next
        col <- vapply(
          seq_len(n), function(k) phi(F[k, ], F[i, ]),
          numeric(1)
        )
  ...
  if (!is.null(query)) {
    ...
    qv <- vapply(
      seq_len(nrow(Q)), function(i) {
        .morie_fsum(w[seq_len(nrow(Cs))] * vapply(
          seq_len(nrow(Cs)),
          function(j) phi(Q[i, ], Cs[j, ]), numeric(1)
        )) + w[length(w)]
      },
      numeric(1)
    )
  ...
  list(
    centers = Cs, weights = w[-length(w)], bias = w[length(w)],
    predictions = fit, queryvalues = qv, mse = mse, spread = spread,
    method = paste0(
      "radial basis function network with greedy forward ",
...
Ahi <- function(airflow, fs, spo2 = NULL, hours = NULL, apneafrac = 0.10,
  ...
  env <- vapply(seq_len(nx), function(i) {
    max(abs(x[max(1L, i - w):min(nx, i + w)]))
  }, numeric(1))
      ...
      if (j - i >= need) {
        ...
        if (ok) {
          events[[length(events) + 1L]] <-
            list(
              kind = kind, start = (i - 1L) / fs, end = (j - 1L) / fs,
  ...
  list(
    ahi = index, severity = sev, apnea = na, hypopnea = nh,
    events = events, hours = hrs, baseline = base, envsec = envsec,
    oxygenchecked = !is.null(ox),
    method = paste0(
      "apnea-hypopnea index from airflow and oximetry ",
...
SparseCode <- function(x, D, sparsity = NULL, lam = NULL, maxiter = 2000,
  ...
  if (!is.null(sparsity)) {
    ...
    r <- BPursuit(x, A,
      lam = as.numeric(lam), maxiter = as.integer(maxiter),
      tol = as.numeric(tol)
    )
    mode <- "lasso"
    src <- paste0(
      "lasso by iterative soft thresholding; Tibshirani, JRSS ",
      "B 58(1):267-288, 1996"
    )
    a <- r$alpha
  }
...
VagTfd <- function(x, fs, natoms = 12, nfreq = 32, ntime = NULL, lag = 12) {
  ...
  tidx <- if (nt > 1L) {
    as.integer(round((seq_len(nt) - 1L) * (n - 1) / (nt - 1)))
  } else {
    0L
  }
  mrange <- (-lag):lag
  ...
  list(
    tfd = tfd, times = times, frequencies = freqs, ep = ep, esp = esp,
    fp = fp, fsp = fsp, coefficients = coef,
    method = paste0(
      "matching-pursuit adaptive TFD of a VAG signal with ",
