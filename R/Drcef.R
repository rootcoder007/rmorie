...
Drcef <- function(y, D, unit, time, cohort, X = NULL) {
  ...
  units <- unique(u); per <- sort(unique(tt)); Tm <- per[length(per)]
  ...
  es <- sort(es)
  ev <- integer(0); theta <- numeric(0); wsum <- numeric(0)
  for (e in es) {
    ok <- cohorts + e <= Tm & (cohorts + e) %in% per & (cohorts - 1) %in% per
    elig <- cohorts[ok]
    if (!length(elig)) next
    tot <- sum(size[ok]); acc <- 0; hit <- FALSE
    ...
    if (!hit) next
    ev <- c(ev, e); theta <- c(theta, acc); wsum <- c(wsum, tot)
  }
  post <- theta[ev >= 0L]
