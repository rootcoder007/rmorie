...
#'
#' Formula: K is a kernel iff the Gram matrix is symmetric positive semi-definite: min
#' eigenvalue(K) >= 0
#'
...
#' doi:10.1007/978-3-030-89010-0.  Chapter 8, Sect. 8.2.1 p. 255, property 2 and property
#' 3: the Gram matrix K with entries K(x_i, x_j) must be positive semi-definite for every
#' choice of x_1, ..., x_n, and the book states that Mercer's theorem -- the integral
#' condition on square-integrable g -- is an equivalent formulation of that finitely
#' positive semi-definite property.  The finite check is therefore the one implemented.
