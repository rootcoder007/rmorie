...
Alfevo <- function(m, z, w, nblock = 1, drop = NULL) {
  ...
  for (blk in seq_len(nblock)) {
    ...
    m <- m + dropm("row", Alfmsaat(m, w$rowq, w$rowk, w$rowv, w$rowg,
                                   w$rowo, z = z, wb = w$rowb, mode = "row")$m)
