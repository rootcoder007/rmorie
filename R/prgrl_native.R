...
.prgrl_cheatsheet <- function() {
  paste0(
    ...
    " proportional to W_lambda(z) P(z) with W_1 = 1; it is a curr",
    "iculum only if H(Q_lambda) increases and W_lambda(z) never f",
    "alls as lambda grows (eqns 3-4), which is checkable and is c",
    "hecked. prgrl trains the same learner on the schedule and on",
    " the shuffled data and compares HELD-OUT error under a fixed",
    " update budget, which is Section 4.2's experiment; easy_only",
    "_fit is Section 4.1's."
  )
}
