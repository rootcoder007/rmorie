...
# Full double precision as PyWavelets tabulates them (db2 agrees with its
# closed form (1 +- sqrt 3, 3 +- sqrt 3) / (4 sqrt 2) to the last bit); the
# earlier table was right to about 12 digits only.

.TF_DBTAPS <- list(
  `1` = c(0.7071067811865476, 0.7071067811865476),
  `2` = c(
    0.48296291314453416, 0.8365163037378079, 0.2241438680420134,
    -0.12940952255126037
  ),
  `3` = c(
  ...
  `6` = c(
    0.11154074335010947, 0.49462389039845306, 0.7511339080210954,
  ...
  `7` = c(
    0.07785205408500918, 0.3965393194819173, 0.7291320908462351,
  ...
  `8` = c(
    0.05441584224310401, 0.31287159091429995, 0.6756307362972898,
    ...
    -0.004870352993451574, -0.00039174037337694705, 0.0006754494064505693,
  ...
  `10` = c(
    0.026670057900555554, 0.1881768000776915, 0.5272011889317256,
    ...
    0.001992405295185056, -0.0006858566949597116, -0.00011646685512928545,
    9.358867032006959e-05, -1.3264202894521244e-05
  )
)
...
OrthFilt <- function(order = 4) {
  ...
  list(
    # convolution convention (PyWavelets, MATLAB wfilters): the analysis
    # filters are the time-reversed synthesis filters
    dec_lo = rev(h), dec_hi = rev(f$g), rec_lo = f$rec_lo, rec_hi = f$rec_hi,
