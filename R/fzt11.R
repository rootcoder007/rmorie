...
#' \deqn{\mathrm{Bias}\[A_h(x)\] = (f'(x) + x^2 f''(x)/2)\sqrt{h} +
#' o(\sqrt{h}),}{Bias\[A_h(x)\] = (f'(x) + x^2 f''(x)/2) sqrt(h) + o(sqrt(h)),}
#' \deqn{\mathrm{Var}\[A_h(x)\] = \frac{R^2(h^{-1/2}-1)
#' f(x)}{2(x+\sqrt{h})\sqrt{\pi}(1-\sqrt{h})R(2h^{-1/2}-2) n h^{1/4}},}{Var\[A_h(x)\] =
#' R^2(h^-1/2 - 1) f(x) / (2(x+sqrt(h)) sqrt(pi) (1-sqrt(h)) R(2h^-1/2 - 2) n h^(1/4)),}
