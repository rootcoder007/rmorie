...
.new_influence_diag <- function(h, cooks, dffits, dfbetas, covratio,
  ...
  structure(
    list(
      hat_values = h, cooks_distance = cooks,
...
.new_collin_diag <- function(vif, cond_num, cond_idx, var_decomp,
  ...
  structure(
    list(
      vif = vif, condition_number = cond_num,
...
.new_spec_test <- function(name, statistic, p_value, df, conclusion) {
  structure(
    list(
      name = name, statistic = statistic, p_value = p_value,
...
compute_influence <- function(y, X, y_hat = NULL) {
  ...
  for (i in seq_len(n)) {
    fit_i <- try(
      .safe_solve(crossprod(X[-i, , drop = FALSE])) %*%
...
compute_vif <- function(X, column_names = NULL) {
  ...
  if (is.null(column_names)) {
    column_names <- if (!is.null(colnames(X))) {
      colnames(X)
    } else {
      paste0("X", seq_len(p) - 1L)
    }
  }
...
collinearity_diagnostics <- function(X, column_names = NULL) {
  ...
  X_scaled <- X / sqrt(matrix(colSums(X^2),
    nrow = n, ncol = p,
    byrow = TRUE
  ) + 1e-10)
  ...
  if (is.null(svdres)) {
    ...
    S2 <- svdres$d^2
    phi <- svdres$v^2
    denom <- rowSums(phi / matrix(S2 + 1e-10,
      nrow = p,
      ncol = p, byrow = TRUE
    ))
    var_decomp <- (phi / matrix(S2 + 1e-10,
      nrow = p, ncol = p,
      byrow = TRUE
    )) /
      pmax(denom, 1e-10)
...
hosmer_lemeshow_test <- function(y, y_prob, n_groups = 10L) {
  ...
  conclusion <- if (p_value < 0.05) {
    ...
    "Adequate fit: no evidence of poor calibration."
  }
...
#' @examples
#' set.seed(1)
#' res <- try(ph_assumption_test(
#'   rexp(60), rbinom(60, 1, 0.7),
...
ph_assumption_test <- function(survival_times, event_indicator,
  ...
  if (is.null(covariate_names)) {
    covariate_names <- if (!is.null(colnames(X))) {
      colnames(X)
    } else {
      paste0("X", seq_len(p) - 1L)
    }
...
full_diagnostics <- function(y, X, y_hat = NULL,
  ...
  .new_diag_report(
    residuals, influence, collinearity,
    gof, spec_tests, assessment
  )
}
