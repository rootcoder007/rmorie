...
KappaCoh <- function(rater1, rater2) {
  ...
  list(
    kappa = kap, p_observed = po, p_expected = pe, se = se, z = z,
    n = n, n_categories = k,
...
KappaWt <- function(rater1, rater2, weights = "linear") {
  ...
  list(
    kappa = 1 - qo / qe, observed_disagreement = qo,
...
CooksD <- function(y, X) {
  ...
  mx <- max(d)
  list(
    d = d, leverage = lev, residual = resid, std_residual = stdres,
...
PnaAgg <- function(A, X,
  ...
  for (i in seq_len(n)) {
    ...
    for (ai in seq_along(aggregators)) {
      per[[ai]] <- vapply(seq_len(f), function(cc) {
        .t2_agg(aggregators[ai], Xm[nb[[i]], cc])
      }, numeric(1))
    }
    aggregated[[i]] <- per
  }
  ...
  list(
    out = out, aggregated = aggregated, degree = deg, delta = delta,
...
RtSi <- function(incidence, serial_interval, window = 7L,
  ...
  list(
    r_mean = r_mean, r_std = r_std, a_posterior = a_post,
