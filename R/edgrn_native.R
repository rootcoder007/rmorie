...
#' @param lib_sample Optional; may be \code{NULL}. Coerced to numeric by the body, with
#' \code{as.numeric}.
#' @param lib_reference Optional; may be \code{NULL}. Coerced to numeric by the body,
...
edgrn_tmm_factor <- function(counts_sample, counts_reference,
  ...
  if (length(y) != length(r)) {
    stop(sprintf(
      "edgrn: %d genes in the sample but %d in the reference",
      length(y), length(r)
    ))
  }
  ...
  list(
    factor = 2^log2f, log2_factor = log2f,
...
edgrn_moderate_dispersion <- function(gene_dispersions, common = NULL,
  ...
  list(
    dispersion = out, common = cval, shrinkage = w,
...
edgrn_exact_test <- function(count_a, count_b, lib_a, lib_b, dispersion) {
  ...
  list(
    p_value = pv, logFC = logFC, dispersion = phi,
...
#' @param df_prior Optional; may be \code{NULL}. Coerced to numeric by the body, with
#' \code{as.numeric}.
...
edgrn_ql_f_test <- function(lrt, q, quasi_dispersion, df_residual,
  ...
  list(
    estimate = Fv, F = Fv, df1 = Q, df2 = d2, p_value = p,
    lrt_p_value = chisq,
