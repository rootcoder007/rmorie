...
.tlonts_cheatsheet <- function() {
  paste0(
    ...
    "each step -- with the conditional law depending on the past ",
    "only through a FIXED-DIMENSIONAL summary and a time-invarian",
    "t mechanism. Effects are defined by STOCHASTIC interventions",
    " on a SUBSET of future treatment nodes, since with a single ",
    "series there is no population to set treatment for. Sample s",
    "ize comes from TIME: the influence terms are a MARTINGALE di",
    "fference sequence, so variance is the sum of squares and the",
    " CLT is the martingale one. If the summary is too small the ",
    "martingale property fails and the interval is simply wrong."
  )
}
