...
.tlstoch_mat <- function(x) {
  ...
  if (is.list(x)) {
    if (length(x) == 0L) return(matrix(numeric(0), nrow=0, ncol=0))
    nr <- length(x)
...
.tlstoch_shift_regime <- function(A, delta, lower=NULL, upper=NULL) {
  ...
  list(shifted=out, delta=d, n_clipped=clipped,
...
.tlstoch_positivity_shift <- function(A, delta, W=NULL, bins=5) {
  ...
  if (is.null(W)) {
    ...
    return(list(fraction_outside=out / n, support=c(lo, hi), delta=d,
                satisfied=(out == 0L)))
  }
  w <- .tlstoch_vec(W)
  lo_w <- min(w)
  ...
  list(fraction_outside=out / n, delta=d, bins=nbins,
       satisfied=(out == 0L),
       note=paste("milder than deterministic positivity: the shift",
...
.tlstoch_stochastic_estimand <- function(Q_fn, A, W, delta,
  ...
  nr <- nrow(W_mat)
  if (nr != length(a)) {
    stop(sprintf("tlstoch: %d exposures but %d covariate rows",
                 length(a), nr))
  }
  ...
  list(psi=mean(vals), observed_mean=mean(obs),
...
.tlstoch_density_ratio <- function(A, W, delta, g_fn,
  ...
  for (i in seq_len(nr)) {
    ...
    if (den <= .tlstoch_eps) {
      stop(sprintf(paste("tlstoch: the observed exposure has zero",
                         "density at observation %d -- the conditional",
                         "density estimate is degenerate"), i))
    }
    out[i] <- num / den
  }
...
.tlstoch_shift_tmle <- function(Y, A, W, Q_fn, g_fn, delta,
  ...
  list(
    ...
    method=paste("TMLE for a stochastic (shift) regime;",
                 "van der Laan & Rose (2018) Chap. 14"),
    note=paste("the clever covariate is a DENSITY RATIO,",
