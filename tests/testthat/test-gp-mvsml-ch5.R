...
test_that("eq (5.1) BLUE/BLUP match the Python anchors", {
  f <- morie_blue_blup_v(X6, Z6, Y6, D2)
...
test_that("eq (5.2) likelihood and REML match the anchors", {
  expect_equal(morie_lmm_loglik(X6, Z6, Y6, D2)$loglik,
               -7.097921931, tolerance = 1e-8)
  expect_equal(morie_reml_loglik(X6, Z6, Y6, D2)$loglik,
               -2.022025101, tolerance = 1e-8)
  # REML = ML + n/2 log(2 pi) - 1/2 log|X'V^-1X|  (book p.146)
  V <- morie_lmm_v(Z6, D2)
  A <- t(X6) %*% solve(V) %*% X6
  expect_equal(morie_reml_loglik(X6, Z6, Y6, D2)$loglik,
               morie_lmm_loglik(X6, Z6, Y6, D2)$loglik +
                 0.5 * 6 * log(2 * pi) -
...
test_that("the EM algorithm matches the Python anchors", {
  r <- morie_em_lmm(X6, Z6, Y6, n_iter = 800L)
...
test_that("eq (5.6) reduces to univariate G x E when all diagonal", {
  ...
  Z_EL <- diag(4)
  G <- diag(2)
  r <- morie_gxe_multitrait(Y, Z_L, Z_EL, G,
...
test_that("kron matches the definition", {
  K <- morie_kron(matrix(c(1,2,3,4), 2, byrow = TRUE),
...
test_that("rank-deficient designs fall back on the pseudo-inverse", {
  ...
  r <- morie_gxe_blup(c(5, 6, 5.4, 6.8), matrix(0, 4, 1),
  ...
  expect_equal(morie_pinv(B), solve(B), tolerance = 1e-12)
})
