...
test_that("eq (5.2) likelihood and REML match the anchors", {
  expect_equal(morie:::morie_mvsml_lmm_loglik(X6, Z6, Y6, D2)$loglik,
               -7.097921931, tolerance = 1e-8)
  expect_equal(morie:::morie_mvsml_reml_loglik(X6, Z6, Y6, D2)$loglik,
               -2.022025101, tolerance = 1e-8)
  # REML = ML + n/2 log(2 pi) - 1/2 log|X'V^-1X|  (book p.146)
  V <- morie:::morie_mvsml_lmm_v(Z6, D2)
  A <- t(X6) %*% solve(V) %*% X6
  expect_equal(morie:::morie_mvsml_reml_loglik(X6, Z6, Y6, D2)$loglik,
               morie:::morie_mvsml_lmm_loglik(X6, Z6, Y6, D2)$loglik +
                 0.5 * 6 * log(2 * pi) -
...
test_that("the EM algorithm matches the Python anchors", {
  r <- morie:::morie_mvsml_em_lmm(X6, Z6, Y6, n_iter = 800L)
...
test_that("eq (5.3) GBLUP matches the Python anchors", {
  ...
  f <- morie:::morie_mvsml_gblup_model(y, Z, G, 0.5)
...
test_that("eq (5.5) reduces to univariate GBLUP when diagonal", {
  ...
  r <- morie:::morie_mvsml_multitrait(Y, Z, G, diag(c(0.4, 0.9)),
                              diag(c(1, 2)))
  for (t in 1:2) {
    s2g <- c(0.4, 0.9)[t]; s2e <- c(1, 2)[t]
    uni <- morie:::morie_mvsml_gblup_model(Y[, t], Z, G, s2g, s2e)
...
test_that("eq (5.6) reduces to univariate G x E when all diagonal", {
  ...
  Z_EL <- diag(4)
  G <- diag(2)
  r <- morie:::morie_mvsml_gxe_multitrait(Y, Z_L, Z_EL, G,
                                  diag(c(0.4, 0.9)), diag(2),
                                  diag(c(0.2, 0.5)), diag(c(1, 2)))
  for (t in 1:2) {
    s2g <- c(0.4, 0.9)[t]; s2ge <- c(0.2, 0.5)[t]
    s2e <- c(1, 2)[t]
    uni <- morie:::morie_mvsml_gxe_blup(Y[, t], NULL, Z_L, Z_EL, G, s2g,
                                diag(s2ge, 2), s2e)
...
test_that("kron matches the definition", {
  K <- morie:::morie_mvsml_kron(matrix(c(1,2,3,4), 2, byrow = TRUE),
...
test_that("rank-deficient designs fall back on the pseudo-inverse", {
  ...
  r <- morie:::morie_mvsml_gxe_blup(c(5, 6, 5.4, 6.8), matrix(0, 4, 1),
