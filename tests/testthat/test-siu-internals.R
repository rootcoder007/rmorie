...
test_that(".siu_write_html_cache + .siu_read_html_cache round-trip", {
  ...
  rmorie:::.siu_write_html_cache(dir, "test.html.gz", html)
  out <- rmorie:::.siu_read_html_cache(dir, "test.html.gz")
...
test_that(".siu_read_html_cache returns empty string on missing file", {
  ...
  dir.create(dir, recursive = TRUE)
  on.exit(unlink(dir, recursive = TRUE), add = TRUE)
  expect_equal(rmorie:::.siu_read_html_cache(dir, "nope.html.gz"), "")
})
...
test_that(".siu_html_to_text strips tags + decodes entities", {
  ...
  out <- rmorie:::.siu_html_to_text(html)
...
test_that(".siu_html_to_text handles empty string", {
  expect_equal(rmorie:::.siu_html_to_text(""), "")
})
...
test_that(".siu_llm_default_timeout returns a positive integer", {
  out <- rmorie:::.siu_llm_default_timeout()
...
test_that(".siu_apply_canonical_overrides handles NULL + 0-row overrides", {
  df <- data.frame(case_number = "24-OFD-001",
                   police_service = "Toronto",
                   stringsAsFactors = FALSE)
  out1 <- rmorie:::.siu_apply_canonical_overrides(df, NULL)
  expect_equal(out1, df)
  out2 <- rmorie:::.siu_apply_canonical_overrides(df,
...
test_that(".siu_load_manifest_raw returns a data.frame or NULL", {
  out <- tryCatch(rmorie:::.siu_load_manifest_raw(),
                  error = function(e) e)
...
test_that(".siu_load_canonical_overrides returns data.frame or NULL", {
  out <- tryCatch(rmorie:::.siu_load_canonical_overrides(),
                  error = function(e) e)
  if (inherits(out, "error")) {
...
test_that(".siu_load_canonical_overrides accepts a user cache dir", {
  ...
  out <- tryCatch(
    rmorie:::.siu_load_canonical_overrides(user_cache),
    error = function(e) e)
  if (inherits(out, "error")) {
...
test_that(".siu_fetch_to_iso parses the canonical 'Month D, YYYY' format", {
  expect_equal(rmorie:::.siu_fetch_to_iso("January 5, 2024"),
               "2024-01-05")
...
test_that(".siu_fetch_resolve_url joins relative + absolute URLs", {
  ...
  expect_match(
    rmorie:::.siu_fetch_resolve_url("/en/news_template.php?nrid=123", base),
    "^https://www\\.siu\\.on\\.ca/")
  expect_equal(
    rmorie:::.siu_fetch_resolve_url("https://other.example/x", base),
...
test_that(".siu_fetch_extract_links returns nrid -> url mapping", {
  ...
  out <- tryCatch(
    rmorie:::.siu_fetch_extract_links(html,
