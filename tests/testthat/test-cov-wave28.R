...
test_that("ARCH-in-mean objective: domain guard + valid path", {
  y <- rnorm(40)
  expect_equal(rmorie:::.archm_negll(c(0, 0, -1, 0.2), y, 40L), 1e10) # omega<=0
  expect_equal(rmorie:::.archm_negll(c(0, 0, 1, 1.5), y, 40L), 1e10) # alpha>=1
  expect_true(is.finite(rmorie:::.archm_negll(c(0, 0, 1, 0.2), y, 40L)))
})

test_that("EGARCH objective: stationarity guard + valid path", {
  r <- rnorm(40)
  EZ <- sqrt(2 / pi)
  expect_equal(rmorie:::.egrch_negll(c(0, 0.1, 0, 1.5), r, 40L, EZ), 1e10)
  expect_true(is.finite(rmorie:::.egrch_negll(c(0, 0.1, 0, 0.5), r, 40L, EZ)))
})
...
test_that("MIDAS SSE objective: theta guard + non-finite guard + valid", {
  ...
  Y <- rnorm(5)
  expect_equal(rmorie:::.midas_sse(c(0, 1, -1, 2), X, Y, 4L), 1e10) # t1<=0
  expect_equal(rmorie:::.midas_sse(c(1e308, 1e308, 1.5, 2), X, Y, 4L), 1e10)
  expect_true(is.finite(rmorie:::.midas_sse(c(0, 1, 1.5, 2), X, Y, 4L)))
})
...
test_that("SAR-lag objective: negative-det + singular guards + valid path", {
  ...
  expect_equal(
    rmorie:::.sarla_negll(1, e0, e1, 4L, I4, diag(c(2, 0, 0, 0))), 1e12
  )
  # W = I, rho = 1  ->  A = 0, singular: determinant() reports sign +1 but
  # modulus -Inf -- the !is.finite(modulus) guard must catch it.
  expect_equal(rmorie:::.sarla_negll(1, e0, e1, 4L, I4, I4), 1e12)
  expect_true(is.finite(
    rmorie:::.sarla_negll(0.2, e0, e1, 4L, I4, diag(c(0.5, 0.5, 0.5, 0.5)))
  ))
})
...
test_that("SAR-error objective: null-beta, det-sign and zero-variance guards", {
  ...
  expect_equal(rmorie:::.sarre_negll(1, I4, I4, X, y, 4L), 1e12)
  # A = diag(-1,1,1,1): full rank, det = -1  ->  the det-sign guard
  expect_equal(
    rmorie:::.sarre_negll(1, I4, diag(c(2, 0, 0, 0)), X, y, 4L), 1e12
  )
...
test_that("spatial-GLM objective: non-PD covariance guard + valid path", {
  ...
  expect_equal(
    rmorie:::.sglm_negll(0, Dbad, 2L, matrix(1, 2, 1), c(1, 2)),
    1e12
  )
  Dok <- as.matrix(dist(1:6))
  expect_true(is.finite(
    rmorie:::.sglm_negll(0, Dok, 6L, cbind(1, 1:6), as.numeric(1:6) + rnorm(6))
  ))
})
...
test_that("spatial-mixed REML objective: covariance + information guards", {
  ...
  expect_equal(rmorie:::.smixd_negreml(
...
test_that("DCC objective: parameter guard + indefinite-correlation guard", {
  ...
  expect_equal(rmorie:::.dccmd_negll(c(-1, 0.5), crossprod(Z) / 2, 2L, Z), 1e10)
  # Q_bar = [[1,2],[2,1]] -> running correlation has determinant -3 (sign -1)
  Z2 <- matrix(rnorm(2), 1, 2)
  expect_equal(
    rmorie:::.dccmd_negll(c(0.02, 0.95), matrix(c(1, 2, 2, 1), 2, 2), 1L, Z2),
    1e10
  )
  ...
  expect_equal(
    rmorie:::.dccmd_negll(c(0.02, 0.95), matrix(1, 2, 2), 1L, Z2), 1e10
  )
})
...
test_that("Horowitz objectives: zero-norm guard + valid path", {
  ...
  ys <- rep(c(-1, 1), 5)
  y <- rnorm(10)
  expect_equal(rmorie:::.hrzb2_loss(c(0, 0), X, ys, 0.5), 1e12)
  expect_true(is.finite(rmorie:::.hrzb2_loss(c(1, 1), X, ys, 0.5)))
  expect_equal(rmorie:::.hrzi1_obj(c(0, 0), X, y, 0.5), 1e12)
  expect_true(is.finite(rmorie:::.hrzi1_obj(c(1, 1), X, y, 0.5)))
})
...
test_that("Horowitz estimators run the negative-coefficient sign flip", {
  ...
  expect_true(is.list(rmorie:::hrzb1(X, yb_neg)))
  expect_true(is.list(rmorie:::hrzb2(X, yb_neg)))
  expect_true(is.list(rmorie:::hrzi1(X, y_neg)))
  expect_true(is.list(rmorie:::hrzc1(X, abs(y_neg) + 0.1)))
})
