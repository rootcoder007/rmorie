...
test_that(".siu_p_stripped_text removes <script>/<style>/tags + decodes entities", {
  ...
  out <- rmorie:::.siu_p_stripped_text(html)
...
test_that(".siu_p_section_text returns '' when header is absent", {
  expect_equal(rmorie:::.siu_p_section_text("body text here",
                                             header = "Mandate"),
               "")
})
...
test_that(".siu_p_section_text slices from header to first end-marker", {
  text <- paste(
    ...
    "Endnotes",
    "Footer", sep = "\n")
  out <- rmorie:::.siu_p_section_text(text,
...
test_that(".siu_p_detect_police_service returns NA on empty / no match", {
  expect_true(is.na(rmorie:::.siu_p_detect_police_service("")))
  expect_true(is.na(rmorie:::.siu_p_detect_police_service(NA)))
})
...
test_that(".siu_p_find_news_release_link returns NA when no template link", {
  out <- rmorie:::.siu_p_find_news_release_link(
...
test_that(".hawkes_restarts returns a 5-element list of phi vectors", {
  phi0 <- c(0, 0, 0, 0)
  out <- rmorie:::.hawkes_restarts(phi0)
...
test_that(".lan_smi_coef builds a coefficient list", {
  out <- rmorie:::.lan_smi_coef(name = "alpha_v",
                                 estimate = 2.4,
                                 std_error = 0.4)
...
test_that(".lan_smi_result builds a morie_laniyonu_smi_result list", {
  alpha_v <- rmorie:::.lan_smi_coef("alpha_v", estimate = 2.4,
                                     std_error = 0.4)
  intercept <- rmorie:::.lan_smi_coef("intercept", -7.5, 0.3)
  out <- rmorie:::.lan_smi_result(
    alpha_v = alpha_v,
    intercept = intercept,
    year_effects = list(),
...
test_that(".tps_result builds a morie_tps_result list", {
  out <- rmorie:::.tps_result(title = "Test",
...
test_that(".tps_vc_rows returns top-N labelled rows w/ percentages", {
  ...
  out <- rmorie:::.tps_vc_rows(x, top = 3L)
  expect_type(out, "list")
...
test_that(".tps_alias_factory captures name via force()", {
  fn <- rmorie:::.tps_alias_factory("Assault")
