...
test_that("eq (8.9) covariance is Z K Z'", {
  ...
  Ks <- morie_kernel_blup_replicated(Z, K)
  expect_equal(Ks[1, 1], 1, tolerance = 1e-12)
  expect_equal(Ks[1, 3], 0.3, tolerance = 1e-12)
  expect_true(morie_is_psd(Ks)$psd)
  expect_equal(morie_kernel_blup_replicated(Z, K, 2)[1, 1], 2,
               tolerance = 1e-12)
})
...
test_that("eq (8.10) interaction kernel is a Hadamard product", {
  ...
  r <- morie_kernel_blup_gxe(Z_u1, K, Z_E)
  ...
  expect_true(morie_is_psd(r$K1)$psd)
  expect_true(morie_is_psd(r$K2)$psd)
})
