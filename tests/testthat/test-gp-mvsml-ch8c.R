...
test_that("eq (8.8) conditional mode equals Henderson's BLUP", {
  ...
  G <- morie_mvsml_grm(M) + diag(0.4, n)
  set.seed(4); y <- rnorm(n, 5, 1)
  s2u <- 0.7; s2e <- 1.3
  fit <- morie_mvsml_blue_blup_v(matrix(1, n, 1), diag(n), y,
                                 s2u * G, diag(s2e, n))
  r <- morie_mvsml_bayesian_kernel_blup(y, G, s2u, s2e,
                                        mu = fit$blue[1])
...
test_that("eq (8.9) covariance is Z K Z'", {
  ...
  Ks <- morie_mvsml_kernel_blup_replicated(Z, K)
  expect_equal(Ks[1, 1], 1, tolerance = 1e-12)
  expect_equal(Ks[1, 3], 0.3, tolerance = 1e-12)
  expect_true(morie_mvsml_is_psd(Ks)$psd)
  expect_equal(morie_mvsml_kernel_blup_replicated(Z, K, 2)[1, 1], 2,
               tolerance = 1e-12)
})
...
test_that("eq (8.10) interaction kernel is a Hadamard product", {
  ...
  r <- morie_mvsml_kernel_blup_gxe(Z_u1, K, Z_E)
  ...
  expect_true(morie_mvsml_is_psd(r$K1)$psd)
  expect_true(morie_mvsml_is_psd(r$K2)$psd)
})

test_that("the Hadamard product matches its definition", {
  A <- matrix(c(1, 3, 2, 4), 2); B <- matrix(c(5, 7, 6, 8), 2)
  expect_equal(morie_mvsml_hadamard(A, B), A * B)
})
