...
test_that("eq (7.6) probabilities normalize and use the baseline", {
  ...
  P <- morie_multinomial_probs(X, c(0.5, -0.5),
                                     matrix(c(1, 0, 0, 1), 2,
                                            byrow = TRUE))
...
test_that("eq (7.11) Poisson pmf and penalized fit", {
  expect_equal(morie_poisson_pmf(2, 3), exp(-3) * 9 / 2,
               tolerance = 1e-12)
  expect_equal(sum(morie_poisson_pmf(0:60, 2.5)), 1,
               tolerance = 1e-10)
  set.seed(8)
  n <- 300
  X <- matrix(runif(n, -1, 1), ncol = 1)
  y <- rpois(n, exp(0.7 + 1.2 * X[, 1]))
  r <- morie_penalized_poisson(X, y, lambda = 0)
  expect_equal(r$beta[1], 0.7, tolerance = 0.3)
  expect_equal(r$beta[2], 1.2, tolerance = 0.3)
  pen <- morie_penalized_poisson(X, y, lambda = 200)
