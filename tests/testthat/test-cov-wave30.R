...
test_that("GARCH / GJR-GARCH objectives: parameter-domain guard + valid", {
  r <- rnorm(40)
  expect_equal(rmorie:::.garch_negll(c(-1, 0.1, 0.8), r, 40L), 1e10) # omega<=0
  expect_equal(rmorie:::.garch_negll(c(0.1, 0.6, 0.6), r, 40L), 1e10) # a+b>=1
  expect_true(is.finite(rmorie:::.garch_negll(c(0.1, 0.1, 0.8), r, 40L)))
  expect_equal(rmorie:::.tgarch_negll(c(-1, 0.1, 0, 0.8), r, 40L), 1e10)
  expect_true(is.finite(rmorie:::.tgarch_negll(
    c(0.1, 0.05, 0.05, 0.8),
    r, 40L
  )))
})
...
test_that("Dirichlet-process / log-spline objectives are finite + valid", {
  expect_true(is.finite(rmorie:::.ghebp_negll(1.5, 5L, 40L)))
  expect_true(is.finite(rmorie:::.ghebp_negll(0.01, 8L, 100L)))
  gz <- seq(-3, 3, length.out = 41)
  basis <- function(u) vapply(seq_len(3), function(k) u^k, numeric(length(u)))
  Bx <- basis(rnorm(30))
  Bg <- basis(gz)
  expect_true(is.finite(rmorie:::.ghlgd_negll(rep(0, 3), Bx, Bg, gz, 30L)))
})

test_that("Manski maximum-score objective returns a bounded score", {
  X <- matrix(rnorm(40), 20, 2)
  ys <- rep(c(-1, 1), 10)
  s <- rmorie:::.hrzb1_score(c(1, -1), ys, X)
...
test_that("variogram model + WLS objective cover every model branch", {
  for (m in c("exponential", "gaussian", "spherical")) {
    expect_true(is.numeric(rmorie:::.vrgft_model(1:3, 0, 1, 2, m)))
  }
  expect_error(
    rmorie:::.vrgft_model(1, 0, 1, 2, "no-such-model"),
    "unknown model"
  )
