...
test_that("the BRR Gibbs sampler recovers a known signal", {
  ...
  r <- morie_mvsml_brr_gibbs(y, X, n_iter = 1500L, burn_in = 400L)
...
test_that("GBLUP is the BRR on the Cholesky factor (p.177)", {
  ...
  G <- morie_mvsml_grm(A) + diag(0.3, n)
  y <- rnorm(n, 5, 1)
  L <- morie_mvsml_chol_lower(G)
  direct <- morie_mvsml_brr_gibbs(y, L, n_iter = 600L,
                                  burn_in = 200L, seed = 3L)
  r <- morie_mvsml_bayes_gblup(y, G, n_iter = 600L,
                               burn_in = 200L, seed = 3L)
...
test_that("the extended predictor stacks blocks in eq (6.6) order", {
  ...
  r <- morie_mvsml_extended_predictor(n, X_E = X_E, X = Xm)
...
test_that("the scaled inverse chi-square has mean S/(nu-2)", {
  set.seed(2)
  d <- morie_mvsml_scaled_inv_chisq(20, 40, n = 4000L)
