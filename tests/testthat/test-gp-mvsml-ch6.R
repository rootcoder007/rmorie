...
test_that("BRR hyperparameters match the BGLR defaults (p.175)", {
  ...
  hp <- morie:::morie_mvsml_brr_hyper(y, R2 = 0.5, nu = 5, nu_beta = 5)
  expect_equal(hp$S, var(y) * 0.5 * 7, tolerance = 1e-12)
  expect_equal(hp$S_beta, var(y) * 0.5 * 7, tolerance = 1e-12)
  hp2 <- morie:::morie_mvsml_brr_hyper(y, sum_var_x = 4)
...
test_that("the BRR Gibbs sampler recovers a known signal", {
  ...
  r <- morie:::morie_mvsml_brr_gibbs(y, X, n_iter = 1500L, burn_in = 400L)
...
test_that("GBLUP is the BRR on the Cholesky factor (p.177)", {
  ...
  G <- morie:::morie_mvsml_grm(A) + diag(0.3, n)
  y <- rnorm(n, 5, 1)
  L <- morie:::morie_mvsml_chol_lower(G)
  direct <- morie:::morie_mvsml_brr_gibbs(y, L, n_iter = 600L,
                                  burn_in = 200L, seed = 3L)
  r <- morie:::morie_mvsml_bayes_gblup(y, G, n_iter = 600L,
                               burn_in = 200L, seed = 3L)
...
test_that("eq (6.5)/(6.7) covariances are Z G Z'", {
  ...
  K <- morie:::morie_mvsml_rkhs_cov(Z, G)$K_L
  expect_equal(K[1, 1], 1, tolerance = 1e-12)
  expect_equal(K[1, 3], 0.4, tolerance = 1e-12)
  full <- morie:::morie_mvsml_rkhs_cov(Z, G, Z_LE = diag(4),
                               I_env = diag(2))
...
test_that("the extended predictor stacks blocks in eq (6.6) order", {
  ...
  r <- morie:::morie_mvsml_extended_predictor(n, X_E = X_E, X = Xm)
...
test_that("the scaled inverse chi-square has mean S/(nu-2)", {
  set.seed(2)
  d <- morie:::morie_mvsml_scaled_inv_chisq(20, 40, n = 4000L)
