...
test_that(".otis_year_col picks EndFiscalYear / Year automatically", {
  ...
  expect_equal(rmorie:::.otis_year_col(df), "EndFiscalYear")
  df2 <- data.frame(Year = 2020:2024, x = 1:5)
  expect_equal(rmorie:::.otis_year_col(df2), "Year")
})

test_that(".otis_to_int coerces text counts to integers + 0 sentinel", {
  expect_equal(rmorie:::.otis_to_int("5"), 5L)
  expect_equal(rmorie:::.otis_to_int(""), 0L)
  expect_equal(rmorie:::.otis_to_int(NA), 0L)
  # Comma-thousands separators are NOT parsed — fall back to 0.
  expect_equal(rmorie:::.otis_to_int("1,000"), 0L)
})

test_that(".otis_is_truthy maps Yes-like values to 1, No-like to 0", {
  # Returns integer (0/1), not logical. NA stays NA.
  expect_equal(rmorie:::.otis_is_truthy("Yes"), 1L)
  expect_equal(rmorie:::.otis_is_truthy("1"), 1L)
  expect_equal(rmorie:::.otis_is_truthy(1L), 1L)
  expect_equal(rmorie:::.otis_is_truthy("No"), 0L)
  expect_equal(rmorie:::.otis_is_truthy("0"), 0L)
  expect_equal(rmorie:::.otis_is_truthy(""), 0L)
  expect_true(is.na(rmorie:::.otis_is_truthy(NA)))
})

test_that(".otis_summary_lines returns the canonical Rows / Years header", {
  df <- make_synthetic_otis("b01", n = 100L, seed = 1L)
  out <- rmorie:::.otis_summary_lines(df, "b01",
...
test_that(".otis_crosstab produces a structured cross-tab", {
  ...
  out <- rmorie:::.otis_crosstab(df, "Gender", "EndFiscalYear",
...
test_that(".otis_c_simple returns a function that runs on c-shaped data", {
  fn <- rmorie:::.otis_c_simple("c04", "test", "Race")
...
test_that(".otis_d_simple returns a function that runs on d-shaped data", {
  fn <- rmorie:::.otis_d_simple("d02", "test", "Gender")
  expect_true(is.function(fn))
...
test_that(".otis_result builds the canonical result list", {
  out <- rmorie:::.otis_result(title = "Test",
                                summary = "Demo result")
  expect_type(out, "list")
})

test_that(".otis_binarise maps Yes/No to 1/0", {
  expect_equal(rmorie:::.otis_binarise(c("Yes", "No", "Yes", NA)),
               c(1L, 0L, 1L, NA))
})
...
test_that(".tps_sp_round rounds with default 3 digits", {
  expect_equal(rmorie:::.tps_sp_round(3.14159), 3.142)
  expect_equal(rmorie:::.tps_sp_round(3.14159, k = 4L), 3.1416)
})
...
test_that(".tps_sp_project_xy returns x/y same length as input", {
  ...
  out <- rmorie:::.tps_sp_project_xy(lat, lon)
  expect_type(out, "list")
...
test_that(".tps_sp_toronto_grid returns nx x ny dim list", {
  out <- rmorie:::.tps_sp_toronto_grid(nx = 10L, ny = 5L)
  expect_type(out, "list")
})

test_that(".tps_sp_roll shifts a matrix along an axis", {
  M <- matrix(1:9, 3L, 3L)
  out0 <- rmorie:::.tps_sp_roll(M, shift = 1L, axis = 1L)
...
test_that(".tps_sp_lap returns Laplacian of a matrix", {
  ...
  out <- rmorie:::.tps_sp_lap(M, dx = 0.1, dy = 0.1)
...
test_that(".tps_sp_grad returns gradient tuple of a matrix", {
  M <- matrix(stats::rnorm(25), 5L, 5L)
  out <- rmorie:::.tps_sp_grad(M, dx = 0.1, dy = 0.1)
  expect_type(out, "list")
})
