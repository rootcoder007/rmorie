...
test_that(".morie_llm_messages builds a system/user message pair", {
  out <- rmorie:::.morie_llm_messages("hello")
...
test_that(".morie_llm_messages embeds context into the system prompt", {
  out <- rmorie:::.morie_llm_messages("q?",
...
test_that(".morie_llm_extract_text returns content from the first choice", {
  ...
  expect_equal(rmorie:::.morie_llm_extract_text(data), "hi")
})
...
test_that(".morie_llm_extract_text returns '' when message is missing", {
  ...
  expect_equal(rmorie:::.morie_llm_extract_text(data), "")
})

test_that(".morie_llm_local_fallback returns the canonical fallback text", {
  out <- rmorie:::.morie_llm_local_fallback("ignored prompt")
...
test_that(".sv_as_matrix returns NULL on NULL input", {
  expect_null(rmorie:::.sv_as_matrix(NULL))
})

test_that(".sv_as_matrix coerces a data.frame to a double matrix", {
  out <- rmorie:::.sv_as_matrix(data.frame(a = 1:3, b = 4:6))
...
test_that(".sv_isotonic_pava no-op on already-monotone input", {
  y <- c(1, 2, 3, 4)
  expect_equal(rmorie:::.sv_isotonic_pava(y), y, tolerance = 1e-10)
})

test_that(".sv_have_cpp returns logical", {
  expect_type(rmorie:::.sv_have_cpp(), "logical")
  expect_type(rmorie:::.sv_have_cpp("nonexistent_cpp_symbol_xyz"),
              "logical")
  expect_false(rmorie:::.sv_have_cpp("nonexistent_cpp_symbol_xyz"))
})
...
test_that(".morie_ckan_sniff_format honours explicit as_format", {
  expect_equal(
    rmorie:::.morie_ckan_sniff_format("https://example/x.csv",
                                       as_format = "JSON"),
    "json")
})
...
test_that(".morie_ckan_sniff_format falls back to extension", {
  expect_equal(
    ...
    "csv")
  expect_equal(
    rmorie:::.morie_ckan_sniff_format("https://example/x.tsv?token=1"),
    "tsv")
})

test_that(".morie_ckan_sniff_format defaults to csv when no extension", {
  expect_equal(
    rmorie:::.morie_ckan_sniff_format("https://example/data"),
    "csv")
})
...
test_that(".morie_ckan_call dispatches with mocked HTTP backend", {
  ...
  out <- rmorie:::.morie_ckan_call("open.toronto.ca", "package_show",
                                    params = list(id = "demo"))
  expect_type(out, "list")
...
test_that(".weighted_prop computes weighted proportion", {
  expect_equal(rmorie:::.weighted_prop(c(1, 0, 1, 0), c(1, 1, 1, 1)),
               0.5)
  expect_equal(rmorie:::.weighted_prop(c(1, 0, 1, 0), c(3, 1, 1, 1)),
               4 / 6, tolerance = 1e-10)
})

test_that(".ess returns effective sample size", {
  # All-equal weights -> ESS = N.
  expect_equal(rmorie:::.ess(rep(1, 10L)), 10)
  # Concentrated weights -> ESS < N.
  out <- rmorie:::.ess(c(rep(0.01, 9L), 100))
...
test_that(".morie_siu_rich builds a morie_siu_result list", {
  out <- rmorie:::.morie_siu_rich(title = "SIU",
...
test_that(".doob_result builds a morie_result list", {
  out <- rmorie:::.doob_result(title = "Doob trends",
...
test_that(".morie_fairness_mean_finite ignores Inf/NaN/NA", {
  expect_equal(rmorie:::.morie_fairness_mean_finite(c(1, 2, 3, NA,
                                                       Inf, NaN)),
               2)
  expect_true(is.na(rmorie:::.morie_fairness_mean_finite(
    c(NA, NaN, Inf))))
})
...
test_that(".morie_statcan_csv_from_zip extracts a CSV from a zip", {
  ...
  out <- rmorie:::.morie_statcan_csv_from_zip(zip_path)
  expect_s3_class(out, "data.frame")
...
test_that(".morie_statcan_csv_from_zip errors on empty archive", {
  ...
  expect_error(rmorie:::.morie_statcan_csv_from_zip(zip_path),
               regexp = "No \\.csv")
})
