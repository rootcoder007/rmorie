...
test_that("robust location estimators match WRS", {
  expect_equal(morie_harrell_davis(X), 46.618013277041733, tolerance = 1e-12)
  expect_equal(morie_harrell_davis(X, 0.25), 23.508793281445108, tolerance = 1e-12)
  expect_equal(morie_mom_estimator(X), 44.75, tolerance = 1e-9)
  expect_equal(morie_one_step_m(X), 50.917616000000002, tolerance = 1e-12)
  expect_equal(morie_pbos(X), 47.714285714285715, tolerance = 1e-12)
})
...
test_that("trimmed-mean inference matches WRS trimse and trimci", {
  expect_equal(morie_trimmed_mean_se(X), 17.01437704011229, tolerance = 1e-12)
  r <- morie_trimmed_mean_ci(X)
  expect_equal(r$estimate, 49.428571428571431, tolerance = 1e-12)
  expect_equal(r$ci[1], 7.7958906092718152, tolerance = 1e-12)
  expect_equal(r$ci[2], 91.061252247871039, tolerance = 1e-12)
  expect_equal(r$p_value, 0.027153064050245534, tolerance = 1e-12)
})

test_that("dependent-groups Yuen matches WRS yuend", {
  r <- morie_yuen_paired(GX, GY8)
  expect_equal(r$estimate, -3.3833333333333364, tolerance = 1e-12)
  expect_equal(r$se, 1.6194649322126944, tolerance = 1e-12)
  expect_equal(r$statistic, -2.0891673947583707, tolerance = 1e-12)
  expect_equal(r$df, 5)
  expect_equal(r$p_value, 0.090996020752312834, tolerance = 1e-12)
...
test_that("rank-based two-group methods match WRS", {
  ...
  expect_equal(cd$delta, -5/9, tolerance = 1e-14)
  expect_equal(cd$ci[1], -0.8511568140198168, tolerance = 1e-12)
  expect_equal(cd$ci[2], 0.007573246151174589, tolerance = 1e-12)
  bm <- morie_brunner_munzel(GX, GY)
  expect_equal(bm$statistic, 2.3959644869352816, tolerance = 1e-12)
  expect_equal(bm$df, 14.68537621214997, tolerance = 1e-12)
  expect_equal(bm$p_value, 0.030374208491219523, tolerance = 1e-12)
})
...
test_that("one-way designs match WRS t1way and bdm", {
  ...
  expect_equal(a$statistic, 17.861507627355412, tolerance = 1e-12)
  expect_equal(a$df1, 2)
  expect_equal(a$df2, 10.628521853986074, tolerance = 1e-12)
  expect_equal(a$p_value, 0.00039906634158703991, tolerance = 1e-12)
...
test_that("boxplot rules match WRS outbox", {
  ...
  expect_equal(b$lower, -70.833333333333314, tolerance = 1e-12)
  expect_equal(b$upper, 175.83333333333331, tolerance = 1e-12)
  cc <- morie_boxplot_outliers(X, carling = TRUE)
  expect_equal(cc$lower, -81.260045489006785, tolerance = 1e-12)
  expect_equal(cc$upper, 171.26004548900679, tolerance = 1e-12)
...
test_that("effect size, median error and Winsorized regression match WRS", {
  ...
  expect_equal(morie_akp_effect_size(GX, GY)$effect_size,
               -0.836340301553, tolerance = 1e-11)
  expect_equal(morie_akp_effect_size(GX, GY, tr = 0)$cterm, 1)
  expect_equal(morie_median_se(X)$se, 23.293468987767866, tolerance = 1e-12)
  expect_equal(morie_median_se(GX)$se, 1.2423183460142859, tolerance = 1e-12)
  w <- morie_winsorized_regression(XS, YS)
  expect_equal(w$intercept, -0.27202587302805831, tolerance = 1e-12)
  expect_equal(unname(w$slope[1]), 2.106668770164883, tolerance = 1e-12)
