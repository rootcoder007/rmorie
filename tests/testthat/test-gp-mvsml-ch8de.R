...
test_that("eq (8.11) design satisfies P P' = K (p.289)", {
  K <- morie_kernel_matrix(Xm, "linear")
  r <- morie_kernel_eigen_design(K)
...
test_that("Nystrom is exact when all lines are landmarks (p.290)", {
  p <- ncol(Xm)
  K <- morie_kernel_matrix(Xm, "linear") / p
  ny <- morie_nystrom(Xm, seq_len(nrow(Xm)))
  expect_equal(ny$Q, K, tolerance = 1e-6)
})

test_that("eq (8.12) design satisfies P P' = Q (p.291)", {
  r <- morie_sparse_kernel_design(Xm, c(1, 4, 6, 8))
...
test_that("eq (8.6) and (8.7) give the same solution (p.276)", {
  ...
  K <- morie_grm(M) + diag(0.5, n)
  C <- matrix(1, n, 1)
  y <- rnorm(n, 5, 1)
  a <- morie_rkhs_mixed_equations(C, K, y, 0.7, 1.3,
                                        "direct")
  b <- morie_rkhs_mixed_equations(C, K, y, 0.7, 1.3,
                                        "reduced")
