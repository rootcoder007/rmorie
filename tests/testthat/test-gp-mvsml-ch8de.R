...
test_that("eq (8.11) design satisfies P P' = K (p.289)", {
  K <- morie:::morie_mvsml_kernel_matrix(Xm, "linear")
  r <- morie:::morie_mvsml_kernel_eigen_design(K)
...
test_that("Nystrom is exact when all lines are landmarks (p.290)", {
  p <- ncol(Xm)
  K <- morie:::morie_mvsml_kernel_matrix(Xm, "linear") / p
  ny <- morie:::morie_mvsml_nystrom(Xm, seq_len(nrow(Xm)))
  expect_equal(ny$Q, K, tolerance = 1e-6)
})

test_that("eq (8.12) design satisfies P P' = Q (p.291)", {
  r <- morie:::morie_mvsml_sparse_kernel_design(Xm, c(1, 4, 6, 8))
...
test_that("eq (8.6) and (8.7) give the same solution (p.276)", {
  ...
  K <- morie:::morie_mvsml_grm(M) + diag(0.5, n)
  C <- matrix(1, n, 1)
  y <- rnorm(n, 5, 1)
  a <- morie:::morie_mvsml_rkhs_mixed_equations(C, K, y, 0.7, 1.3,
                                        "direct")
  b <- morie:::morie_mvsml_rkhs_mixed_equations(C, K, y, 0.7, 1.3,
                                        "reduced")
