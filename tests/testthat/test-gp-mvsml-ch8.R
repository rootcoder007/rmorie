...
test_that("kernels are symmetric and positive semi-definite", {
  for (k in c("linear", "gaussian", "polynomial", "exponential")) {
    K <- morie_kernel_matrix(X, kernel = k)
    expect_equal(K, t(K), tolerance = 1e-12)
    expect_true(morie_is_psd(K)$psd)
  }
  bad <- matrix(c(1, 2, 2, 1), 2)
  expect_false(morie_is_psd(bad)$psd)
})
...
test_that("eq (8.2) representer form matches the definition", {
  ...
  pred <- morie_rkhs_predict(K, beta, 0.5)
  expect_equal(pred, as.numeric(0.5 + K %*% beta), tolerance = 1e-12)
  expect_equal(morie_rkhs_norm(beta, K),
               sum(outer(beta, beta) * K), tolerance = 1e-12)
})

test_that("eq (8.3) fit shrinks with lambda", {
  K <- morie_kernel_matrix(X, kernel = "gaussian", gamma = 0.5)
  small <- morie_rkhs_fit(K, Y, lambda = 0.01)
  large <- morie_rkhs_fit(K, Y, lambda = 10)
  expect_lt(morie_rkhs_norm(large$beta, K),
            morie_rkhs_norm(small$beta, K))
  expect_gt(small$penalty, 0)
})

test_that("eq (8.4) arc-cosine kernel properties (p.265)", {
  K <- morie_arccos_kernel(XA)
  expect_equal(diag(K), rowSums(XA^2), tolerance = 1e-12)
  expect_equal(morie_arccos_kernel(matrix(c(1, 2), 1),
                                         Z = matrix(c(-1, -2), 1))[1],
               0, tolerance = 1e-12)
  # orthogonal unit inputs: theta = pi/2, J = 1, so AK = 1/pi
  expect_equal(morie_arccos_kernel(matrix(c(1, 0), 1),
                                         Z = matrix(c(0, 1), 1))[1],
               1 / pi, tolerance = 1e-12)
  expect_true(morie_is_psd(K)$psd)
  # heterogeneous diagonal, unlike the Gaussian kernel
  expect_gt(diff(range(diag(K))), 1e-6)
  G <- morie_kernel_matrix(XA, kernel = "gaussian")
  expect_lt(diff(range(diag(G))), 1e-12)
})

test_that("eq (8.5) deep recursion reduces to (8.4) at depth 1", {
  a <- morie_arccos_kernel(XA, depth = 1L)
  b <- morie_arccos_kernel(XA, depth = 2L)
  expect_true(morie_is_psd(b)$psd)
  expect_false(isTRUE(all.equal(a, b)))
  expect_equal(morie_arccos_kernel(XA, depth = 1L), a,
               tolerance = 1e-12)
  # median normalization as in the book's R code
  K <- morie_arccos_kernel(XA, normalize_median = TRUE)
