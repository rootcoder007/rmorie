...
test_that("eq (6.10) ridge form reproduces G (p.194)", {
  ...
  G <- morie:::morie_mvsml_grm(A) + diag(0.4, 6)
  r <- morie:::morie_mvsml_multitrait_ridge(diag(6), G)
...
test_that("the inverse-Wishart draw has mean S/(nu-p-1)", {
  set.seed(3)
  nu <- 30; S <- diag(c(4, 9))
  d <- replicate(600, morie:::morie_mvsml_inv_wishart(nu, S),
                 simplify = FALSE)
...
test_that("BMTME conditionals follow the p.196 Gibbs steps", {
  ...
  r <- morie:::morie_mvsml_bmtme_conditionals(Y, diag(J), Z2, G,
                                      matrix(c(1, .2, .2, 1), 2),
                                      diag(2), diag(2),
                                      b1 = b1, b2 = b2)
  ...
  r0 <- morie:::morie_mvsml_bmtme_conditionals(Y, diag(J), Z2, G,
                                       matrix(c(1, .2, .2, 1), 2),
                                       diag(2), diag(2), b1 = b1)
