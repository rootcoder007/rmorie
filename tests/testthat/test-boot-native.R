...
test_that("morie_boot is seed-reproducible", {
  x <- rnorm(40)
  set.seed(7); a <- morie_boot(x, function(d, i) median(d[i]), R = 100)
  set.seed(7); c <- morie_boot(x, function(d, i) median(d[i]), R = 100)
...
test_that("morie_boot_ci returns ordered intervals for all four types", {
  ...
  expect_gte(mean(x), ci$perc[1]); expect_lte(mean(x), ci$perc[2])
})

test_that("morie_tsboot runs for moving-block and stationary schemes", {
  set.seed(3); y <- as.numeric(arima.sim(list(ar = 0.4), 100))
...
test_that("morie_two_boot bootstraps the mean difference", {
  set.seed(4); a <- rnorm(30, 5); c <- rnorm(25, 4)
