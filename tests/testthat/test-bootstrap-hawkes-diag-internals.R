...
test_that(".new_bootstrap_result builds a morie_bootstrap_result list", {
  out <- rmorie:::.new_bootstrap_result(
    estimate = 0.5, se = 0.1,
...
test_that(".new_jackknife_result builds a morie_jackknife_result list", {
  out <- rmorie:::.new_jackknife_result(
    estimate = 0.5, se = 0.1,
    ci_lower = 0.3, ci_upper = 0.7, bias = 0.01,
...
test_that(".new_permutation_test_result has class + default NA CI", {
  out <- rmorie:::.new_permutation_test_result(
...
test_that(".pct equals stats::quantile type-7 linear interp", {
  x <- 1:100
  expect_equal(rmorie:::.pct(x, 50), 50.5)
  expect_equal(rmorie:::.pct(x, 95), 95.05)
})

test_that(".idx subsets a vector by integer index", {
  expect_equal(rmorie:::.idx(c(10, 20, 30, 40), c(1L, 3L)),
               c(10, 30))
})

test_that(".idx subsets a matrix preserving columns", {
  M <- matrix(1:12, nrow = 4L, ncol = 3L)
  out <- rmorie:::.idx(M, c(1L, 3L))
  expect_true(is.matrix(out))
  expect_equal(dim(out), c(2L, 3L))
})

test_that(".nrow_like uses nrow for matrix/df, length otherwise", {
  expect_equal(rmorie:::.nrow_like(matrix(1:9, 3L, 3L)), 3L)
  expect_equal(rmorie:::.nrow_like(data.frame(a = 1:5, b = 6:10)), 5L)
  expect_equal(rmorie:::.nrow_like(c(1, 2, 3, 4, 5)), 5L)
})

# =============================================================== hawkes_fit.R

test_that(".hawkes_param_names returns 3-vec for exp / 4-vec otherwise", {
  expect_equal(rmorie:::.hawkes_param_names("exponential"),
               c("a0", "eta", "beta"))
  expect_equal(rmorie:::.hawkes_param_names("weibull"),
               c("a0", "eta", "alpha", "lambda"))
  expect_equal(rmorie:::.hawkes_param_names("lomax"),
               c("a0", "eta", "alpha", "c"))
  expect_equal(rmorie:::.hawkes_param_names("gamma"),
               c("a0", "eta", "alpha", "beta"))
})

test_that(".hawkes_param_names errors on unknown kernel", {
  expect_error(rmorie:::.hawkes_param_names("garch"),
               regexp = "unknown kernel")
})

test_that(".hawkes_to_theta + .hawkes_to_phi round-trip", {
  phi <- c(-1.0, 0.5, log(2), log(3))
  theta <- rmorie:::.hawkes_to_theta(phi)
  back <- rmorie:::.hawkes_to_phi(theta)
  expect_equal(back, phi, tolerance = 1e-10)
})

test_that(".hawkes_to_theta maps phi[2] -> (0,1) via plogis", {
  theta <- rmorie:::.hawkes_to_theta(c(0, 0, 0, 0))
  # plogis(0) = 0.5
...
test_that(".hawkes_loglik_poisson matches the closed-form", {
  # logLik = n * log(n/T) - n
  expect_equal(rmorie:::.hawkes_loglik_poisson(100L, 1000),
...
test_that(".hawkes_start returns the right-length init for each kernel", {
  ...
  expect_length(rmorie:::.hawkes_start("gamma", t, 100), 4L)
})

test_that(".hawkes_kernel_funs returns g + G closures for exp kernel", {
  funs <- rmorie:::.hawkes_kernel_funs("exponential",
                                        c(0, 0.5, 2.0))
...
test_that(".hawkes_kernel_funs returns NULL on infeasible param", {
  expect_null(rmorie:::.hawkes_kernel_funs("exponential",
                                           c(0, 0.5, 0)))
})

test_that(".hawkes_nll_pureR returns penalty on infeasible eta", {
  out <- rmorie:::.hawkes_nll_pureR(theta = c(0, 0, 1.0),
                                     times = c(1, 2, 3),
                                     end_time = 10,
                                     kernel = "exponential")
...
test_that(".kernel_gaussian peaks at u = 0", {
  expect_equal(rmorie:::.kernel_gaussian(0),
               1 / sqrt(2 * pi), tolerance = 1e-10)
  # Symmetric around 0.
  expect_equal(rmorie:::.kernel_gaussian(1),
               rmorie:::.kernel_gaussian(-1), tolerance = 1e-10)
})

test_that(".kernel_epanechnikov is 0 outside [-1,1] and 0.75 at u=0", {
  expect_equal(rmorie:::.kernel_epanechnikov(0), 0.75)
  expect_equal(rmorie:::.kernel_epanechnikov(1.5), 0)
  expect_equal(rmorie:::.kernel_epanechnikov(-1.5), 0)
})

test_that(".kernel_uniform is 0.5 inside [-1,1] and 0 outside", {
  expect_equal(rmorie:::.kernel_uniform(0), 0.5)
  expect_equal(rmorie:::.kernel_uniform(0.5), 0.5)
  expect_equal(rmorie:::.kernel_uniform(1.5), 0)
})

test_that(".kernel_triangular is 1 at u=0 and 0 outside [-1,1]", {
  expect_equal(rmorie:::.kernel_triangular(0), 1)
  expect_equal(rmorie:::.kernel_triangular(0.5), 0.5)
  expect_equal(rmorie:::.kernel_triangular(1.5), 0)
})

test_that(".kernel_biweight is (15/16) at u=0 and 0 outside [-1,1]", {
  expect_equal(rmorie:::.kernel_biweight(0), 15 / 16,
               tolerance = 1e-10)
  expect_equal(rmorie:::.kernel_biweight(1.5), 0)
})
...
test_that(".kernel_fn returns each numbered kernel + Gaussian fallback", {
  ...
  expect_equal(rmorie:::.kernel_fn(0L)(0), 1 / sqrt(2 * pi),
               tolerance = 1e-10)
  expect_equal(rmorie:::.kernel_fn(1L)(0), 0.75)
  expect_equal(rmorie:::.kernel_fn(2L)(0), 0.5)
  expect_equal(rmorie:::.kernel_fn(3L)(0), 1)
  expect_equal(rmorie:::.kernel_fn(4L)(0), 15 / 16, tolerance = 1e-10)
  expect_equal(rmorie:::.kernel_fn(5L)(0), 1 / sqrt(2 * pi),
               tolerance = 1e-10)
})

test_that(".resolve_kernel maps a name to a code via .kernel_name_map", {
  out <- rmorie:::.resolve_kernel("gaussian")
  expect_type(out, "integer")
})

test_that(".resolve_kernel passes a numeric code through", {
  expect_equal(rmorie:::.resolve_kernel(2), 2L)
})

# ============================================================== diagnostics.R

test_that(".new_residual_diag stamps morie_residual_diagnostics", {
  out <- rmorie:::.new_residual_diag(
...
test_that(".new_spec_test stamps morie_specification_test", {
  out <- rmorie:::.new_spec_test(name = "RESET", statistic = 2.5,
                                  p_value = 0.04, df = 2L,
...
test_that(".new_gof stamps morie_goodness_of_fit", {
  out <- rmorie:::.new_gof(r_squared = 0.8, adj_r_squared = 0.78,
                            pseudo_r_squared = NA,
                            aic = 100, bic = 110,
...
test_that(".siu_p_has_rvest returns logical", {
  expect_type(rmorie:::.siu_p_has_rvest(), "logical")
})

test_that(".siu_p_blank_row returns the canonical NA-row template", {
  row <- rmorie:::.siu_p_blank_row()
...
test_that(".siu_p_re_escape escapes regex metacharacters", {
  esc <- rmorie:::.siu_p_re_escape("a.b+c*d?")
...
test_that(".siu_p_parse_drid_from_url extracts numeric drid", {
  expect_equal(
    rmorie:::.siu_p_parse_drid_from_url(
...
test_that(".siu_p_parse_nrid_from_url extracts numeric nrid", {
  expect_equal(
    rmorie:::.siu_p_parse_nrid_from_url(
      "https://www.siu.on.ca/en/news_template.php?nrid=7777"),
    7777L)
  expect_true(is.na(rmorie:::.siu_p_parse_nrid_from_url(NULL)))
})
...
test_that(".siu_p_normalise_sex maps synonyms to canonical labels", {
  ...
  expect_equal(rmorie:::.siu_p_normalise_sex("x"),     "Non-binary")
...
test_that(".siu_p_label_value extracts a labelled value", {
  ...
  out <- rmorie:::.siu_p_label_value(text, "Location of call")
...
test_that(".siu_p_label_int extracts the first integer after the label", {
  ...
  expect_equal(rmorie:::.siu_p_label_int(text, "Number of officers involved"),
               3L)
  expect_true(is.na(rmorie:::.siu_p_label_int("no match", "label")))
})

test_that(".siu_p_detect_language returns 'en' / 'fr' / 'unknown'", {
  out <- rmorie:::.siu_p_detect_language("plain ASCII text with no markers")
