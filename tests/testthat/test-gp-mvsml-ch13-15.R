...
test_that("eq. 13.1 convolution matches Python", {
  ...
  r <- morie:::morie_mvsml_conv2d(img, ker, bias = 0.5)
...
test_that("eq. 14.3/14.9 design and 14.4 fit match Python", {
  ...
  d <- morie:::morie_mvsml_fda_design(tg, curves, L1 = 3, L2 = 5)
  ...
  expect_gt(min(eigen(crossprod(d$X_star), only.values = TRUE)$values),
            1e-8)
  y <- 1 + 0.5 * (0:9)
  f <- morie:::morie_mvsml_fda_fit(tg, curves, y, L1 = 3, L2 = 5)
...
test_that("eq. 14.6 coefficients and 14.8 LOOCV match Python", {
  ...
  Psi <- morie:::morie_mvsml_fda_basis(tg, 5)
  expect_equal(morie:::morie_mvsml_fda_coefficients(Psi, x), c(1, -0.5, 0.4, 4.60742555219e-17, -1.02140518266e-17),
               tolerance = 1e-7)
  # eq. (14.6) rebuilds the curve from those coefficients
  expect_equal(as.numeric(Psi %*% morie:::morie_mvsml_fda_coefficients(Psi, x)),
  ...
  expect_equal(morie:::morie_mvsml_fda_loocv(tg, x, 5), 9.58342740327e-31, tolerance = 1e-7)
})

test_that("eq. 15.1-15.3 ZAP quantities match Python", {
  z <- morie:::morie_mvsml_zap_link(c(0.1, 0.5, -0.3), c(-1, 0, 2))
  ...
  expect_equal(morie:::morie_mvsml_zap_loglik(c(1, 2, 3, 1, 4)), -7.29052929945,
               tolerance = 1e-9)
  p <- morie:::morie_mvsml_zap_predict(c(0.2, 0.6, 0.1), c(1.5, 3.0, 0.8))
...
test_that("the eq. 15.3 erratum is corrected in R too", {
  ...
  for (par in list(c(0.2, 1.5), c(0.6, 3.0), c(0.05, 0.4))) {
    ...
    expect_equal(morie:::morie_mvsml_zap_predict(th, mu)$prediction, direct,
                 tolerance = 1e-9)
    mv <- morie:::morie_mvsml_zap_mean_variance(th, mu)
    expect_equal(mv$mean, direct, tolerance = 1e-9)
  }
  # eq. (15.4): 0 when theta > 0.5, mu-hat otherwise (not the ZAP mean)
  cl <- morie:::morie_mvsml_zap_predict(c(0.7, 0.5, 0.2), c(4, 4, 4),
                                threshold = 0.5)
...
test_that("marginal structural models match Python", {
  ...
  w <- 1 + 0.05 * ((0:39) %% 7)
  expect_equal(morie:::morie_msm_linear(y, A, w)$beta, c(0.999563318777, 2.0015312509), tolerance = 1e-9)

  yb <- as.numeric((A[, 1] + A[, 2] + ((0:39) %% 3 == 0)) >= 2)
  expect_equal(morie:::morie_msm_logistic(yb, A)$beta, c(-24.4936371443, 24.4936373522), tolerance = 1e-6)

  yc <- ((0:39) %% 4) + rowSums(A)
  expect_equal(morie:::morie_msm_poisson(yc, A)$beta, c(-4.4408920985e-16, 0.69314718056), tolerance = 1e-6)

  tt <- 1 + ((0:39) %% 9) * 0.5
  ev <- as.numeric((0:39) %% 4 != 0)
  expect_equal(morie:::morie_msm_cox_marginal(tt, ev, A)$beta, 0.337507401646,
               tolerance = 1e-7)
  a <- morie:::morie_msm_accelerated_failure(tt, ev, A)
  expect_equal(a$beta, c(0.95592351286, -0.00161346302844), tolerance = 1e-9)
  expect_equal(a$n_uncensored, 30)
  expect_equal(morie:::morie_msm_gmm_estimator(y, A, weights = w)$beta, c(0.999563318777, 2.0015312509),
               tolerance = 1e-9)
})
...
test_that("the MSM weights change the estimate they are meant to", {
  ...
  expect_equal(unname(morie:::morie_msm_linear(y, A)$beta),
               unname(coef(lm(y ~ rowSums(A)))), tolerance = 1e-9)
  # doubling every weight cannot move a weighted least squares fit
  expect_equal(morie:::morie_msm_linear(y, A, rep(2, 6))$beta,
               morie:::morie_msm_linear(y, A)$beta, tolerance = 1e-9)
  # the Cox MSM agrees with a weighted Newton step on the same data
  tt <- c(1, 2, 3, 4, 5, 6); ev <- c(1, 1, 1, 0, 1, 1)
  expect_true(is.finite(morie:::morie_msm_cox_marginal(tt, ev, A)$hazard_ratio))
})
