...
test_that("cosine_lr_schedule returns a decayed learning rate", {
  out <- tryCatch(
    rmorie:::cosine_lr_schedule(x = 50L, lr_max = 1e-3, lr_min = 0,
                       total_steps = 100L),
    error = function(e) e)
  if (inherits(out, "error"))
...
test_that("word_embedding returns an embedding matrix", {
  set.seed(1L)
  out <- tryCatch(
    rmorie:::word_embedding(x = sample.int(50L, 20L, replace = TRUE),
...
test_that("flash_attention runs on small Q/K/V matrices", {
  ...
  out <- tryCatch(
    rmorie:::flash_attention(Q, K, V, block_size = 8L),
    error = function(e) e)
  if (inherits(out, "error"))
...
test_that("bpe_tokenizer learns BPE merges from a short corpus", {
  set.seed(3L)
  out <- tryCatch(
    rmorie:::bpe_tokenizer(x = c("hello world", "world of morie",
...
test_that(".na_omit_cols drops rows with NAs in named cols", {
  df <- data.frame(x = c(1, NA, 3, 4), y = c("a", "b", NA, "d"),
                   z = 1:4)
  out <- rmorie:::.na_omit_cols(df, c("x", "y"))
