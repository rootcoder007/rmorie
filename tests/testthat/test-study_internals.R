...
# internal helpers (accessed via rmorie:::) — pure-fn / numeric paths
# that don't need a full CPADS pipeline run.

# ============================================================== study_core

test_that(".wald_ci returns symmetric (1.96 SE) bounds", {
  ci <- rmorie:::.wald_ci(estimate = 0.5, se = 0.1)
...
test_that(".binary_ci returns plausible Wilson/Wald CI", {
  out <- rmorie:::.binary_ci(successes = 30L, n = 100L)
...
test_that(".binary_ci handles 0/N and N/N edge cases", {
  out_zero <- rmorie:::.binary_ci(successes = 0L, n = 100L)
  out_full <- rmorie:::.binary_ci(successes = 100L, n = 100L)
...
test_that(".weighted_binary_estimate computes p-hat + SE for a weighted sample", {
  ...
  out <- rmorie:::.weighted_binary_estimate(x, w)
  expect_true(is.list(out) || is.numeric(out))
  flat <- unlist(out)
...
test_that(".clip_exp clips its argument to a safe exp() range", {
  out_pos <- rmorie:::.clip_exp(1000)        # clip to ~exp(50)
  out_neg <- rmorie:::.clip_exp(-1000)       # clip to ~exp(-50)
  out_mid <- rmorie:::.clip_exp(1.5)         # passes through
...
test_that(".or_table runs on a real logistic fit", {
  ...
  out <- tryCatch(rmorie:::.or_table(fit, model = "logit"),
                  error = function(e) e)
...
test_that(".cpads_labeled_data adds value labels to a synthetic CPADS frame", {
  ...
  out <- tryCatch(rmorie:::.cpads_labeled_data(df),
                  error = function(e) e)
  if (inherits(out, "error")) {
...
test_that(".block_schedule generates a block schedule for a given N", {
  out <- tryCatch(
    rmorie:::.block_schedule(endpoint = "binary",
                             required_n = 200L,
...
test_that(".read_existing_output returns the fallback on missing file", {
  ...
  out <- rmorie:::.read_existing_output(tmp, "nonexistent.json",
