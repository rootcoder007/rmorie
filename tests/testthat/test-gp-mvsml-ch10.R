...
test_that("forward pass matches eq (10.1)-(10.3)", {
  W <- mknet()
  f <- morie_mvsml_ann_forward(X, W, c("logistic", "identity"))
  z <- sum(W[[1]][1, ] * X[1, ])
...
test_that("eq (10.5) SSE matches the formula", {
  expect_equal(morie_mvsml_ann_sse(matrix(c(0.5, 0.2)),
                                   matrix(c(1, 0))),
...
test_that("backprop gradients match central differences", {
  ...
  for (hid in c("logistic", "tanh")) {
    acts <- c(hid, "identity")
    ana <- morie_mvsml_ann_gradients(X, Y, W, acts)$gradients
    num <- morie_mvsml_ann_numeric_gradient(X, Y, W, acts)
...
test_that("training decreases the loss for a small learning rate", {
  W <- mknet(7)
  r <- morie_mvsml_ann_train(X, Y, W, eta = 0.05, n_iter = 3000L,
