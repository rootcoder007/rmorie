...
test_that(".hrz_silverman returns 1.0 on n < 2 (degenerate fallback)", {
  expect_equal(rmorie:::.hrz_silverman(c(5)), 1.0)
})
...
test_that(".hrz_gauss_kernel peaks at 1/sqrt(2*pi) at u=0", {
  expect_equal(rmorie:::.hrz_gauss_kernel(0),
               1 / sqrt(2 * pi), tolerance = 1e-12)
  # Symmetric.
  expect_equal(rmorie:::.hrz_gauss_kernel(1.0),
               rmorie:::.hrz_gauss_kernel(-1.0), tolerance = 1e-12)
})
...
test_that(".hrz_hermite returns n x J orthonormalised basis matrix", {
  H <- rmorie:::.hrz_hermite(seq(-2, 2, length.out = 25L), J = 4L)
...
test_that(".val_result stamps the requested class hierarchy", {
  out <- rmorie:::.val_result(class_name = "morie_cv_summary",
                              n = 10L, score = 0.85)
...
test_that(".val_auc returns 1 on perfectly separable data", {
  ...
  expect_equal(rmorie:::.val_auc(y_true, y_pred), 1.0)
})
...
test_that(".val_score dispatches scoring -> metric", {
  ...
  expect_true(rmorie:::.val_score("brier", y, p) < 0)
})

test_that(".val_score errors on unknown scoring", {
  expect_error(rmorie:::.val_score("unknown", c(0, 1), c(0.5, 0.5)),
...
test_that(".is_boolean_value_set detects yes/no, true/false, 0/1", {
  expect_true(rmorie:::.is_boolean_value_set(c("yes", "no")))
  expect_true(rmorie:::.is_boolean_value_set(c("True", "False")))
  expect_true(rmorie:::.is_boolean_value_set(c("0", "1")))
  expect_false(rmorie:::.is_boolean_value_set(c("maybe", "yes")))
  expect_false(rmorie:::.is_boolean_value_set(NULL))
})

test_that(".cardinality_from_vv steps through the n thresholds", {
  expect_equal(rmorie:::.cardinality_from_vv(NULL), "unknown")
  expect_equal(rmorie:::.cardinality_from_vv(c("a", "b")), "binary")
  expect_equal(rmorie:::.cardinality_from_vv(letters[1:5]),
               "discrete_low")
  expect_equal(rmorie:::.cardinality_from_vv(letters[1:20]),
               "discrete_medium")
  expect_equal(rmorie:::.cardinality_from_vv(letters[1:26]),
               "discrete_medium")
})

test_that(".level_from_spec identifies identifiers / dates / booleans", {
  expect_equal(
    rmorie:::.level_from_spec("Record_ID", "int", NULL, "otis"),
    "identifier")
  expect_equal(
    rmorie:::.level_from_spec("placement_date", "date", NULL, "otis"),
    "date")
  expect_equal(
    rmorie:::.level_from_spec("MentalHealth_Alert", "bool", NULL, "otis"),
    "boolean")
})

test_that(".level_from_spec ratio vs nominal heuristics", {
  expect_equal(
    rmorie:::.level_from_spec("number_of_days", "int", NULL, "otis"),
    "ratio")
  expect_equal(
    rmorie:::.level_from_spec("severity_level", "int", NULL, "otis"),
    "ordinal")
})

test_that(".role_from_name identifies identifier / outcome / covariate", {
  expect_equal(rmorie:::.role_from_name("Record_ID"), "identifier")
  expect_equal(rmorie:::.role_from_name("injury_status"), "outcome")
  expect_equal(rmorie:::.role_from_name("officer_age"), "covariate")
})

# ============================================================= tps_stochastic.R

test_that(".tps_stoch_result builds a stochastic-result list", {
  out <- rmorie:::.tps_stoch_result(title = "Test", call = "demo",
...
test_that(".tps_stoch_round rounds to k digits + NA on non-finite", {
  expect_equal(rmorie:::.tps_stoch_round(3.14159, 2L), 3.14)
  expect_true(is.na(rmorie:::.tps_stoch_round(Inf, 2L)))
  expect_true(is.na(rmorie:::.tps_stoch_round(NA, 2L)))
})
...
test_that(".tps_stoch_date_series uses OCC_YEAR/MONTH/DAY triple", {
  df <- data.frame(OCC_YEAR  = c(2020, 2021, 2022),
                   OCC_MONTH = c(1, 6, 12),
                   OCC_DAY   = c(15, 1, 31))
  ts <- rmorie:::.tps_stoch_date_series(df, min_year = 2014L)
  expect_s3_class(ts, "POSIXct")
  expect_length(ts, 3L)
})

test_that(".tps_stoch_daily on empty input returns zero-row list", {
  out <- rmorie:::.tps_stoch_daily(as.POSIXct(character(0), tz = "UTC"))
...
test_that(".tps_stoch_monthly on empty input returns zero-row list", {
  out <- rmorie:::.tps_stoch_monthly(as.POSIXct(character(0), tz = "UTC"))
  expect_named(out, c("dates", "counts"))
  expect_length(out$counts, 0L)
})
...
test_that(".tps_stoch_neg_loglik_hawkes returns 1e12 on infeasible params", {
  ...
  out <- rmorie:::.tps_stoch_neg_loglik_hawkes(
...
test_that(".frns_check_aligned passes on equal-length inputs", {
  expect_silent(rmorie:::.frns_check_aligned(
    list("a", c(1, 2, 3)),
    list("b", c(4, 5, 6))))
})

test_that(".frns_check_aligned errors on length mismatch", {
  expect_error(rmorie:::.frns_check_aligned(
    list("a", c(1, 2)),
    list("b", c(4, 5, 6))),
...
test_that(".frns_check_aligned errors on empty inputs", {
  expect_error(rmorie:::.frns_check_aligned(list("a", integer(0))),
               regexp = "empty")
})
...
test_that(".frns_favorable_rates computes per-group rates", {
  ...
  group   <- c("A", "A", "A", "B", "B", "B")
  out <- rmorie:::.frns_favorable_rates(outcome, group, "hire")
...
test_that(".frns_resolve_privileged passes explicit + infers when NULL", {
  ...
  out1 <- rmorie:::.frns_resolve_privileged("B", rates)
  expect_equal(out1$privileged, "B")
  expect_null(out1$warning)
  out2 <- rmorie:::.frns_resolve_privileged(NULL, rates)
...
test_that(".frns_resolve_privileged errors on unknown explicit group", {
  ...
  expect_error(rmorie:::.frns_resolve_privileged("Z", rates),
               regexp = "not found")
})
...
test_that(".frns_rates_from_labels computes per-group TPR + FPR", {
  ...
  y_pred <- c(1, 0, 0, 1)
  group  <- c("A", "A", "B", "B")
  out <- rmorie:::.frns_rates_from_labels(y_true, y_pred, group, 1)
  expect_named(out, c("A", "B"))
  expect_true(is.numeric(out$A$tpr))
})

test_that(".frns_gini returns 0 on degenerate input + >0 on skew", {
  expect_equal(rmorie:::.frns_gini(rep(5, 10L)), 0)
  expect_equal(rmorie:::.frns_gini(numeric(0)), 0)
  expect_true(rmorie:::.frns_gini(c(rep(0, 99L), 1000)) > 0.9)
})

test_that(".frns_worst_abs returns the largest-magnitude finite value", {
  expect_equal(rmorie:::.frns_worst_abs(c(0.1, -0.5, 0.3, NaN)), -0.5)
  expect_true(is.na(rmorie:::.frns_worst_abs(c(NaN, Inf, -Inf))))
})

# =============================================================== siu_analyze.R

test_that(".siu_an_truthy counts true-like values", {
  expect_equal(rmorie:::.siu_an_truthy(c(TRUE, FALSE, TRUE)), 2L)
  expect_equal(rmorie:::.siu_an_truthy(c("yes", "no", "Yes", "1")), 3L)
})

test_that(".siu_an_falsy counts false-like values", {
  expect_equal(rmorie:::.siu_an_falsy(c(TRUE, FALSE, FALSE)), 2L)
  expect_equal(rmorie:::.siu_an_falsy(c("no", "yes", "false", "0")), 3L)
})

test_that(".siu_an_interval returns 'n/a' on empty / NA dates", {
  out <- rmorie:::.siu_an_interval("label",
                                    character(0),
                                    character(0))
