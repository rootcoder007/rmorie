...
test_that("GEV / GP log-density helpers cover the xi ~ 0 + support branches", {
  ...
  expect_length(rmorie:::.extvm_log_gev(c(0, 0, 0), x), 20)
  expect_length(rmorie:::.gpfit_log_gp(c(0, 0), abs(x) + 0.1), 20)
  # xi large + data far from the location -> 1 + xi z <= 0 (out of support)
  oos <- rmorie:::.extvm_log_gev(c(0, 0, 5), c(-100, -200))
  expect_true(all(oos <= -1e9))
  oos2 <- rmorie:::.gpfit_log_gp(c(0, -5), c(50, 100))
  expect_true(all(oos2 <= -1e9))
  # generic xi != 0 path
  expect_length(rmorie:::.extvm_log_gev(c(0, 0, 0.2), x), 20)
  expect_length(rmorie:::.gpfit_log_gp(c(0, 0.2), abs(x) + 0.1), 20)
})
...
test_that("kernel callables recompute a non-positive supplied bandwidth", {
  ...
  ok("hrzi2", rmorie:::hrzi2(X, y, bandwidth = -1))
  ok("hrzk1", rmorie:::hrzk1(y, bandwidth = -1))
  ok("hrzk2", rmorie:::hrzk2(X[, 1], y, bandwidth = -1))
  ok("hrzk3", rmorie:::hrzk3(X[, 1], y, bandwidth = -1))
  ok("hrzp1", rmorie:::hrzp1(X[, 1], y, z, bandwidth = -1))
  ...
  ok("hrzk3_far", rmorie:::hrzk3(X[, 1], y, grid = 1e6))
  expect_true(TRUE)
})
...
test_that("backprop activation switch stops on an unknown activation", {
  expect_error(
    rmorie:::.bkprp_sigma_prime(0.5, "not-an-activation", 0.5),
    "Unknown activation"
  )
})
...
test_that("Horowitz single-index handles a near-zero lm.fit coefficient", {
  ...
  ok("hrzi1_zeronorm", rmorie:::hrzi1(X, rep(1, 60)))
  expect_true(TRUE)
})
...
test_that("mrm SIU / TPS empty-stratum branches return cleanly", {
  ok("siu_km_empty", rmorie:::mrm_siu_case_to_decision_km(data.frame(
    case_number = "C1", incident_date = NA, decision_date = NA
  )))
  ok("tps_recur", rmorie:::mrm_tps_neighbourhood_recurrence_km(data.frame(
