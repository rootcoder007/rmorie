...
test_that("influence and residual diagnostics equal base R's influence.measures", {
  x1 <- c(2.1, 2.9, 2.0, 3.3, 2.6, 3.0, 2.4, 2.7, 2.5, 3.1, 1.9, 2.2, 4.8, 2.8)
  x2 <- c(1.0, 3.0, 2.0, 5.0, 4.0, 2.5, 3.5, 1.5, 4.5, 3.2, 2.2, 1.1, 2.0, 3.9)
  y <- c(5.1, 6.3, 4.8, 7.2, 5.9, 6.6, 5.4, 6.0, 5.5, 6.8, 4.2, 5.0, 12.0, 6.1)
  X <- cbind(1, x1, x2)
  f <- stats::lm(y ~ x1 + x2)
  inf <- compute_influence(y, X)
  r <- compute_residuals(y, stats::fitted(f), X)
  expect_equal(unname(r$studentized_residuals), unname(stats::rstudent(f)), tolerance = 1e-12)
  expect_equal(unname(r$standardized_residuals), unname(stats::rstandard(f)), tolerance = 1e-12)
  expect_equal(unname(inf$dffits), unname(stats::dffits(f)), tolerance = 1e-12)
  expect_equal(unname(inf$dfbetas), unname(stats::dfbetas(f)), tolerance = 1e-12)
  expect_equal(unname(inf$covratio), unname(stats::covratio(f)), tolerance = 1e-12)
  expect_equal(unname(inf$cooks_distance), unname(stats::cooks.distance(f)), tolerance = 1e-12)
})
