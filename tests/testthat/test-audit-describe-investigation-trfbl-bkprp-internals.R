...
test_that(".classify_schema_R returns list-of-taxonomies for a schema", {
  ...
  out <- rmorie:::.classify_schema_R(schema, dataset_name = "demo")
...
test_that(".summarise_taxonomies returns counts + flag lists", {
  taxonomies <- list(
    ...
    list(column_name = "x1",  level = "nominal",
         ...
         cross_year_safe = TRUE))
  out <- rmorie:::.summarise_taxonomies(taxonomies,
                                         analyzed_set = c("y"),
                                         domain = "test")
  expect_type(out, "list")
})
...
test_that(".morie_normalise_describe_name strips morie_ / describe_ / .md", {
  expect_equal(
    rmorie:::.morie_normalise_describe_name("morie_alpha"), "alpha")
  expect_equal(
    rmorie:::.morie_normalise_describe_name("describe_alpha.md"), "alpha")
  expect_equal(
    rmorie:::.morie_normalise_describe_name("alpha"), "alpha")
})

test_that(".morie_load_describe_corpus returns a non-NULL object (or errors when bundle absent)", {
  out <- tryCatch(rmorie:::.morie_load_describe_corpus(),
                  error = function(e) e)
...
test_that(".morie_hajek_ate returns ate + y1 + y0 list", {
  ...
  out <- rmorie:::.morie_hajek_ate(ps, t, y)
...
test_that(".morie_fit_propensity clips ps to [0.01, 0.99]", {
  ...
  df <- data.frame(d = stats::rbinom(n, 1L, 0.5),
                   x1 = stats::rnorm(n),
                   x2 = stats::rnorm(n))
  ps <- rmorie:::.morie_fit_propensity(df, "d", c("x1", "x2"))
  expect_length(ps, n)
...
test_that(".vrgft_model dispatches all three variogram families", {
  ...
  expect_equal(rmorie:::.vrgft_model(0, c0 = 0.5, c1 = 1, a = 10,
                                      model = "exponential"), 0.5)
  expect_equal(rmorie:::.vrgft_model(1e6, c0 = 0.5, c1 = 1, a = 10,
                                      model = "exponential"), 1.5)
  # spherical at h > a saturates at c0 + c1.
  expect_equal(rmorie:::.vrgft_model(20, c0 = 0.5, c1 = 1, a = 10,
                                      model = "spherical"), 1.5)
})

test_that(".vrgft_model errors on unknown model", {
  expect_error(rmorie:::.vrgft_model(1, 0, 1, 10, "garch"),
...
test_that(".vrgft_obj returns >=0 WLS objective", {
  ...
  weights <- c(1, 1, 1)
  out <- rmorie:::.vrgft_obj(p = c(0, 1, 2), mids, gammas, weights,
                             model = "exponential")
...
test_that(".trfbl_gelu approximates 0 at z=0 and z at z>>0", {
  expect_equal(rmorie:::.trfbl_gelu(0), 0, tolerance = 1e-12)
  # For large z, GELU(z) ~ z.
  expect_equal(rmorie:::.trfbl_gelu(10), 10, tolerance = 1e-3)
  # For large negative z, GELU(z) ~ 0.
  expect_equal(rmorie:::.trfbl_gelu(-10), 0, tolerance = 1e-3)
})
...
test_that(".bkprp_sigma dispatches identity/linear/none + saturating", {
  ...
  expect_equal(rmorie:::.bkprp_sigma(z, "identity"), z)
  expect_equal(rmorie:::.bkprp_sigma(z, "linear"),   z)
  expect_equal(rmorie:::.bkprp_sigma(z, "none"),     z)
  expect_equal(rmorie:::.bkprp_sigma(0, "sigmoid"), 0.5)
  expect_equal(rmorie:::.bkprp_sigma(0, "tanh"),    0)
  # NB: relu via pmax(0, .) strips matrix dim attributes; assert via vec.
  expect_equal(rmorie:::.bkprp_sigma(c(-1, 0, 1, 2), "relu"),
               c(0, 0, 1, 2))
})

test_that(".bkprp_sigma errors on unknown activation", {
  expect_error(rmorie:::.bkprp_sigma(matrix(0, 1, 1), "wat"),
               regexp = "Unknown activation")
})

test_that(".bkprp_sigma_prime returns shape-correct gradients", {
  z <- matrix(c(-1, 0, 1, 2), 2L, 2L)
  a <- rmorie:::.bkprp_sigma(z, "sigmoid")
  out <- rmorie:::.bkprp_sigma_prime(z, "sigmoid", a)
  expect_equal(dim(out), dim(z))
  # ReLU derivative is (z>0); a passed but unused on this branch.
  rd <- rmorie:::.bkprp_sigma_prime(z, "relu", a)
...
test_that(".tgarch_negll returns 1e10 on infeasible parameters", {
  r <- stats::rnorm(20L)
  out <- rmorie:::.tgarch_negll(p = c(-0.1, 0, 0, 0), r = r, n = 20L)
...
test_that(".morie_tps_features_to_rows returns empty list on empty input", {
  out <- rmorie:::.morie_tps_features_to_rows(list(),
...
test_that(".morie_tps_features_to_rows extracts attributes per feature", {
  ...
  out <- rmorie:::.morie_tps_features_to_rows(features,
                                               return_geometry = FALSE)
  expect_type(out, "list")
...
test_that(".tps_csi_to_long passes data.frame through with required cols", {
  df <- data.frame(year = c(2020, 2021),
                   category = c("a", "b"),
                   count = c(5L, 7L))
  out <- rmorie:::.tps_csi_to_long(df, "year")
...
test_that(".tps_csi_to_long errors on data.frame missing required cols", {
  ...
  expect_error(rmorie:::.tps_csi_to_long(df, "year"),
               regexp = "long-format")
})
...
test_that(".fuzzy_match_key returns NULL when no catalog entry matches", {
  expect_null(rmorie:::.fuzzy_match_key("nonexistent_key_xyz"))
})
...
test_that(".fuzzy_match_key resolves a hyphen / case variation to a real catalog key", {
  ...
  out <- rmorie:::.fuzzy_match_key(munged)
