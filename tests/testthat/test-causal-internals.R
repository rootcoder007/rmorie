...
test_that(".fit_propensity returns ps in (0,1) on synthetic data", {
  ...
  ps <- rmorie:::.fit_propensity(df, "d", c("x1", "x2"))
  expect_true(is.numeric(ps))
  expect_true(all(ps > 0 & ps < 1))
})

test_that(".clip_ps clips ps to [eps, 1-eps]", {
  expect_equal(rmorie:::.clip_ps(c(0.0, 0.5, 1.0), eps = 0.01),
               c(0.01, 0.5, 0.99))
  expect_equal(rmorie:::.clip_ps(c(-1, 2)), c(1e-6, 1 - 1e-6))
})

test_that(".hajek_diff computes weighted-mean difference", {
  out <- rmorie:::.hajek_diff(y1 = c(1, 2, 3), w1 = c(1, 1, 1),
                              y0 = c(0, 1), w0 = c(1, 1))
...
test_that(".bootstrap_ci returns CI bounds on a simple statistic", {
  ...
  ci <- tryCatch(
    rmorie:::.bootstrap_ci(function(x) mean(x), args, n_boot = 50L),
    error = function(e) e)
...
test_that(".tipping_point_result builds a tipping-point result list", {
  out <- rmorie:::.tipping_point_result(
...
test_that(".morie_rdd_get_kernel returns a kernel function or weight vector", {
  out <- tryCatch(rmorie:::.morie_rdd_get_kernel("triangular"),
                  error = function(e) e)
  if (inherits(out, "error")) {
...
test_that(".morie_rdd_local_poly_fit fits a local polynomial near cutoff", {
  ...
  out <- tryCatch(
    rmorie:::.morie_rdd_local_poly_fit(x, y, x0 = 0, h = 2.0),
    error = function(e) e)
  if (inherits(out, "error")) {
...
test_that(".morie_rdd_bw_result builds a bandwidth result list", {
  out <- rmorie:::.morie_rdd_bw_result(h = 2.5, method = "imbens-kalyanaraman")
  expect_type(out, "list")
})

test_that(".morie_rdd_result builds an RDD estimate result list", {
  out <- rmorie:::.morie_rdd_result(estimate = 1.5, se = 0.3, n = 100L,
                                     method = "local-linear")
  expect_type(out, "list")
})
