...
test_that(".tbl_fmt_num formats a numeric with default 2 digits", {
  expect_equal(rmorie:::.tbl_fmt_num(3.14159), "3.14")
})

test_that(".tbl_fmt_num returns '' on non-finite", {
  expect_equal(rmorie:::.tbl_fmt_num(NA_real_), "")
  expect_equal(rmorie:::.tbl_fmt_num(Inf), "")
})

test_that(".tbl_fmt_num apa style strips leading zero for |x| < 1", {
  expect_equal(rmorie:::.tbl_fmt_num(0.42, apa = TRUE), ".42")
  expect_equal(rmorie:::.tbl_fmt_num(-0.42, apa = TRUE), "-.42")
})

test_that(".tbl_fmt_pval formats a normal-range p-value", {
  expect_equal(rmorie:::.tbl_fmt_pval(0.045, digits = 3L), "0.045")
})

test_that(".tbl_fmt_pval renders tiny p as '<0.001'", {
  expect_equal(rmorie:::.tbl_fmt_pval(1e-9, digits = 3L), "<0.001")
})

test_that(".tbl_stars steps through ***/**/* thresholds", {
  expect_equal(rmorie:::.tbl_stars(0.0005), "***")
  expect_equal(rmorie:::.tbl_stars(0.005),  "**")
  expect_equal(rmorie:::.tbl_stars(0.04),   "*")
  expect_equal(rmorie:::.tbl_stars(0.5),    "")
  expect_equal(rmorie:::.tbl_stars(NA),     "")
})

test_that(".tbl_smd matches the canonical (m1-m2)/sqrt((s1^2+s2^2)/2)", {
  out <- rmorie:::.tbl_smd(m1 = 5, m2 = 3, sd1 = 1, sd2 = 1)
  expect_equal(out, 2 / sqrt(1), tolerance = 1e-10)
})

test_that(".tbl_smd returns 0 when pooled sd is ~0", {
  expect_equal(rmorie:::.tbl_smd(5, 3, 0, 0), 0)
})
...
test_that(".tbl_footnotes_add appends a unique note", {
  reg <- rmorie:::.tbl_footnotes_new()
  rmorie:::.tbl_footnotes_add(reg, "note A")
  rmorie:::.tbl_footnotes_add(reg, "note B")
  rmorie:::.tbl_footnotes_add(reg, "note A")  # dup -> no-op
  expect_length(reg$notes, 2L)
})

test_that(".tbl_footnotes_render returns '' on empty registry", {
  reg <- rmorie:::.tbl_footnotes_new()
  expect_equal(rmorie:::.tbl_footnotes_render(reg), "")
})

test_that(".tbl_footnotes_render emits LaTeX / HTML / text variants", {
  reg <- rmorie:::.tbl_footnotes_new()
  rmorie:::.tbl_footnotes_add(reg, "see appendix")
  expect_match(rmorie:::.tbl_footnotes_render(reg, fmt = "latex"),
               "textsuperscript")
  expect_match(rmorie:::.tbl_footnotes_render(reg, fmt = "html"),
               "<sup>")
  expect_match(rmorie:::.tbl_footnotes_render(reg, fmt = "text"),
               "see appendix")
})
...
test_that(".tbl_to_format renders csv with title + footnotes", {
  df <- data.frame(a = 1:2)
  out <- rmorie:::.tbl_to_format(df, "csv",
...
test_that(".morie_url_with_params is a no-op on NULL/empty params", {
  expect_equal(
    rmorie:::.morie_url_with_params("https://example.com/x", NULL),
    "https://example.com/x")
  expect_equal(
    rmorie:::.morie_url_with_params("https://example.com/x", list()),
    "https://example.com/x")
})

test_that(".morie_url_with_params appends query string", {
  out <- rmorie:::.morie_url_with_params(
...
test_that(".morie_detect_format extension fallback covers known types", {
  for (ext_fmt in list(c("data.json", "json"), c("d.tsv", "tsv"),
    ...
    out <- rmorie:::.morie_detect_format(
...
test_that(".nz returns the first non-empty/non-NA string", {
  expect_equal(rmorie:::.nz(NULL, NA, "", "found", "next"), "found")
...
test_that(".morie_resolve_path expands ~ + normalises", {
  out <- rmorie:::.morie_resolve_path("~/morie_test_path")
...
test_that(".morie_dataset_pkg_csv returns NA for unknown name", {
  expect_true(is.na(rmorie:::.morie_dataset_pkg_csv("__nonexistent__")))
})
...
test_that(".morie_dataset_year_where formats numeric year", {
  expect_equal(rmorie:::.morie_dataset_year_where(2024),
               "OCC_YEAR = 2024")
})

test_that(".morie_dataset_records_to_df passes data.frame through", {
  df <- data.frame(a = 1:3)
  expect_identical(rmorie:::.morie_dataset_records_to_df(df), df)
})

test_that(".morie_dataset_records_to_df returns 0-row frame on empty input", {
  out <- rmorie:::.morie_dataset_records_to_df(list())
...
test_that(".na_omit_cols drops rows with NA in the chosen cols", {
  ...
  out <- rmorie:::.na_omit_cols(df, c("a"))
  expect_equal(nrow(out), 2L)
})
...
test_that(".safe_divide returns NA on zero or NA denom", {
  ...
  expect_equal(rmorie:::.safe_divide(4, 2), 2)
})

test_that(".wald_ci returns symmetric 1.96-SE bounds", {
  expect_equal(rmorie:::.wald_ci(0, 1), c(-1.96, 1.96))
  expect_equal(rmorie:::.wald_ci(5, 0), c(5, 5))
})

test_that(".binary_ci returns p, se, ci on a small sample", {
  out <- rmorie:::.binary_ci(successes = 30, n = 100)
...
test_that(".weighted_binary_estimate handles ordinary all-1 weights", {
  out <- rmorie:::.weighted_binary_estimate(
    x = c(1, 0, 1, 0, 1),
    w = c(1, 1, 1, 1, 1))
...
test_that(".clip_exp clamps |x| to 700 before exp() to avoid overflow", {
  ...
  expect_equal(rmorie:::.clip_exp(1e6), exp(700), tolerance = 1e-10)
})
