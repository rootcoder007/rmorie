...
# (.morie_did_*, .morie_matching_*) accessed via rmorie:::. These
...
test_that(".morie_did_make_ci returns symmetric CI bounds", {
  out <- rmorie:::.morie_did_make_ci(estimate = 0.5, se = 0.1)
...
test_that(".morie_did_add_intercept prepends a 1-column", {
  ...
  out <- rmorie:::.morie_did_add_intercept(X)
...
test_that(".morie_did_drop_na drops rows with NAs in named cols", {
  df <- data.frame(x = c(1, NA, 3), y = c("a", "b", NA),
                   z = 1:3)
  out <- rmorie:::.morie_did_drop_na(df, c("x", "y"))
  expect_equal(nrow(out), 1L)
})

test_that(".morie_did_pvalue returns 2-sided p from a t stat", {
  expect_lt(rmorie:::.morie_did_pvalue(1.96), 0.06)
  expect_gt(rmorie:::.morie_did_pvalue(0.0), 0.99)
})
...
test_that(".morie_did_ols_robust_se with cluster_ids returns clustered SE", {
  ...
  out <- tryCatch(
    rmorie:::.morie_did_ols_robust_se(X, y, cluster_ids = clust),
    error = function(e) e)
  if (inherits(out, "error")) {
...
test_that(".morie_matching_logit returns ln(p/(1-p)) with eps clipping", {
  expect_equal(rmorie:::.morie_matching_logit(0.5), 0)
  expect_lt(rmorie:::.morie_matching_logit(0.01), -3)
  expect_gt(rmorie:::.morie_matching_logit(0.99), 3)
...
test_that(".morie_matching_drop_na drops rows with NAs in named cols", {
  df <- data.frame(x = c(1, NA, 3), y = c("a", "b", NA),
                   z = 1:3)
  out <- rmorie:::.morie_matching_drop_na(df, c("x", "y"))
  expect_equal(nrow(out), 1L)
})

test_that(".morie_matching_empty_pairs returns the empty-pairs sentinel", {
  out <- rmorie:::.morie_matching_empty_pairs()
