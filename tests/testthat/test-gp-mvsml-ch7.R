...
test_that("eq (7.1) probabilities match the book formula", {
  p <- morie_mvsml_ordinal_probs(0, c(-1, 0.5))
  ...
  hi <- morie_mvsml_ordinal_probs(2, c(-1, 0.5))
  expect_gt(hi[1], p[1])
  expect_lt(hi[3], p[3])
})

test_that("the logistic link matches the p.210 formula", {
  p <- morie_mvsml_ordinal_probs(0.3, c(-0.5, 1), link = "logistic")
...
test_that("the binary case reduces to probit regression (p.210)", {
  p <- morie_mvsml_ordinal_probs(0.4, 0)
...
test_that("truncated normal draws stay inside the interval", {
  set.seed(4)
  d <- replicate(500, morie_mvsml_rtruncnorm(0, 1, 0.5, 1.5))
...
test_that("the ordinal probit Gibbs sampler recovers a signal", {
  ...
  r <- morie_mvsml_ordinal_probit_gibbs(y, X, n_iter = 900L,
                                        burn_in = 300L)
